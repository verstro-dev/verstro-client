...
APP_VERSION="$(sed -n -E 's/^version:[[:space:]]*([^+[:space:]]+)\+([0-9]+)[[:space:]]*$/\1/p' "$ROOT/pubspec.yaml" | head -n 1)"
APP_BUILD="$(sed -n -E 's/^version:[[:space:]]*([^+[:space:]]+)\+([0-9]+)[[:space:]]*$/\2/p' "$ROOT/pubspec.yaml" | head -n 1)"
[[ -n "$APP_VERSION" && -n "$APP_BUILD" ]] || {
  echo "无法从 pubspec.yaml 读取版本和 build" >&2
  exit 1
}
export APP_BUILD
...
reset_fixtures() {
  ...
  for abi in arm64-v8a armeabi-v7a x86_64; do
    write_meta "$abi" com.verstro.app "$APP_VERSION" "$EXPECTED_SIGNER"
  done
}

run_verifier() {
  AAPT_BIN="$TMP/bin/aapt" APKSIGNER_BIN="$TMP/bin/apksigner" \
    "$VERIFIER" --apk-dir "$TMP/apks" --tag "v$APP_VERSION"
}
...
for abi in arm64-v8a armeabi-v7a x86_64; do
  write_meta "$abi" com.verstro.app "$APP_VERSION" "$EXPECTED_SIGNER" build_tools_37
done
run_verifier | grep -Fq 'Android 三 ABI 正式发布校验通过'
...
reset_fixtures
write_meta arm64-v8a com.verstro.app "$APP_VERSION" "$EXPECTED_SIGNER" \
  build_tools_37_distinct_signers "$EXPECTED_SIGNER" \
  AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
expect_failure 'signer 证书数量异常: 2' run_verifier

reset_fixtures
write_meta armeabi-v7a com.verstro.app.dev "$APP_VERSION" "$EXPECTED_SIGNER"
...
reset_fixtures
write_meta arm64-v8a com.verstro.app "$APP_VERSION" AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
