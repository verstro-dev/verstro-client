...
class Tray {
  ...
  String getTryIcon({required bool isStart, required bool tunEnable}) {
    if (system.isMacOS) {
      // macOS 会把模板图标的 Alpha 通道当作菜单栏图形。通用状态图标带有
      // 不透明圆角底板，会被系统渲染成纯色方块，因此必须使用透明底的专用图标。
      return 'assets/images/icon/status_macos.png';
    }
    if (!isStart) {
  ...
  Future<void> update({
    ...
    for (final mode in ModeExt.visibleValues) {
      menuItems.add(
        MenuItem.checkbox(
          label: mode.label,
          onClick: (_) {
