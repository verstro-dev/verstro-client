...
class System {
  ...
  // Verstro iOS: 上游 System 漏了 isIOS (只有 isAndroid). iOS 端核心走
  // Network Extension + sing-box libbox, 接入点要靠 isIOS 分流 (controller /
  // 订阅 UA / profile 校验). 见 decisions why-ios-singbox-network-extension.
  ...
  Future<bool> checkIsAdmin() async {
    ...
    if (system.isWindows) {
      ...
      // 必须用绝对路径 /usr/bin/stat: 若用户 PATH 里有 Homebrew coreutils,
      // 裸 `stat` 会解析到 GNU stat, 其 -f 是 --file-system (而非 BSD 的格式标志),
      // 会把 '%Su:%Sg %Sp' 当文件名报错 → 此检查永远 false → _requestAdmin
      // 反复走 authorizeCore success 分支 → osascript 提权框无限循环.
      final result =
          await Process.run('/usr/bin/stat', ['-f', '%Su:%Sg %Sp', corePath]);
