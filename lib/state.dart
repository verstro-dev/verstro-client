...
class GlobalState {
  ...
  Future<ProviderContainer> _initData(int version) async {
    ...
    config = await _applyAutoRunOffOnce(config);
    if (planStartupProxyHeal(
      autoRun: config.appSettingProps.autoRun,
    ).stopProxy) {
  ...
  Future<Config> _applyAutoRunOffOnce(Config config) async {
    final prefs = await preferences.sharedPreferencesCompleter.future;
    final plan = planAutoRunOffOnce(
      alreadyMigrated: prefs?.getBool(kVerstroAutoRunOffV1) ?? false,
      currentAutoRun: config.appSettingProps.autoRun,
    );
    var next = config;
    if (next.appSettingProps.autoRun != plan.autoRun) {
      next = next.copyWith(
        appSettingProps: next.appSettingProps.copyWith(autoRun: plan.autoRun),
      );
      await preferences.saveConfig(next);
    }
    if (plan.writeMigrationMark) {
      await prefs?.setBool(kVerstroAutoRunOffV1, true);
    }
    return next;
  }
}
