...
class CoreService extends CoreHandlerInterface {
  ...
  Future<void> start() async {
    ...
    if (system.isWindows && await system.checkIsAdmin()) {
      ...
      if (isSuccess) {
        // helper 异步拉起核心进程; 必须等核心回连 socket 再返回, 否则上层立即
        // applyProfile / 读节点会赶在核心就绪前 → 首次节点列表为空(需重启才好)。
        // 超时兜底: 不无限等(防 helper 声称成功但核心未真正起来时挂死), 超时按
        // 原行为返回, 无回归。
        try {
          await _socketCompleter.future.timeout(const Duration(seconds: 5));
        } catch (_) {}
        return;
      }
    }
