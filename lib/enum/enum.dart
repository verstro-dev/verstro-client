...
extension ModeExt on Mode {
  /// 面向终端用户的模式名称(私有服务，仅展示智能分流/全局代理)。
  String get label => switch (this) {
    Mode.rule => appLocalizations.vAppModeRule,
    Mode.global => appLocalizations.vAppModeGlobal,
    Mode.direct => appLocalizations.direct,
  };

  /// 展示给终端用户的模式(隐藏 direct，私有 VPN 无需纯直连入口)。
  static const List<Mode> visibleValues = [Mode.rule, Mode.global];
}
...
enum PageLabel {
  ...
  resources,
  connections,
  // Verstro: 账户/套餐/订单 一级 tab (阶段 2.3.8). 见 common/navigation.dart
  account,
}
