...
class ApplicationState extends ConsumerState<Application> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      if (planApplicationFirstFrame(hasNavigator: currentContext != null) ==
      ...
      if (updateCtx != null && updateCtx.mounted) {
        ...
        // Verstro 升级检查: 后台、非阻塞、fail-open. 拉 R2 manifest 比对版本,
        // 有更新按类型弹窗(已忽略的 optional/silent 版本不打扰).
        if (updateCtx.mounted) {
  ...
  Future<void> _verstroAutoIntegrate({bool forceRefresh = true}) async {
    try {
      ...
      if (plan.action == VerstroProfileAction.import) {
        ...
        if (idx != -1) {
          ...
          plan.action == VerstroProfileAction.refresh) {
  ...
  Widget build(context) {
    ref.listen(authNotifierProvider, (prev, next) {
      ...
      if (nextId != null && nextId != prevId) {
        unawaited(_verstroAutoIntegrate(forceRefresh: true));
      }
    });
    ref.listen(subscriptionProvider, (prev, next) {
      ...
      final url = next.asData?.value.subscriptionUrl;
  ...
  Future<void> dispose() async {
    appController.isAttach = false;
    linkManager.destroy();
    _autoUpdateProfilesTaskTimer?.cancel();
    await proxy?.stopProxy();
    await coreController.destroy();
    if (planApplicationDispose() == ApplicationDisposeCleanup.selfCoreAndExit) {
