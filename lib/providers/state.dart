...
NavigationItemsState navigationItemsState(Ref ref) {
  ...
  return NavigationItemsState(
    value: navigation.getItems(
      openLogs: openLogs,
      // iOS: 核心在 NE 扩展、组数据经 CommandClient 异步到达且连接慢; 连上瞬间 isInit→true 而
      // currentGroups 可能还空 → 代理 tab 闪没。iOS 改为"有 profile 即常驻代理 tab"(空也展示,
      // 顶部刷新按钮可刷; 组就绪后自动填充)。见真机调试 + 记忆 project_ios_device_debug。
      hasProxies: system.isIOS
          ? hasProfiles
          : (!isInit ? hasProfiles : hasProxies),
    ),
  );
}
