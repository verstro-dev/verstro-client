...
GroupsState currentGroupsState(Ref ref) {
  ...
  return GroupsState(
    value: switch (mode) {
      Mode.direct => [],
      // 私有服务：两种模式都只展示真实线路选择器，隐藏内核 GLOBAL 与 hidden 组。
...
NavigationItemsState navigationItemsState(Ref ref) {
  ...
  // 代理 tab 显隐一律按"是否有 profile", 不再按 currentGroups 是否已加载 (全平台统一)。
  // 原桌面逻辑 (!isInit ? hasProfiles : hasProxies) 在 init 完成但组尚未加载时会隐藏
  // 代理 tab —— Windows 覆盖安装升级后首启, 核心组加载晚于 initProvider 置 true (且
  // applyProfile 若因 TUN 提权早返回会跳过 updateGroups), 就出现"代理 tab 不显示、
  // 点连接后才出现"。组本就是异步到达 (桌面升级/重启后更慢, iOS 更是走 NE 扩展),
  // 用是否有 profile 判定更稳: 有订阅即常驻代理 tab, 组就绪后自动填充, 顶部刷新按钮可刷。
  // iOS 早已因同样原因这么做 (见记忆 project_ios_device_debug), 此处收敛为全平台一致。
...
SharedState sharedState(Ref ref) {
  ...
  // 把 GLOBAL 钉到主线路组：覆盖 Android 冷启动 quickSetup(读缓存的 sharedState)路径，
  // 否则上次处于全局模式时冷启会短暂走直连。只 select 组名，避免随延迟测试等频繁重算落盘。
  final primary = ref.watch(
    groupsProvider.select((state) => state.primarySelectorName),
  );
  final currentProfileName = currentProfileVM2.a;
  final selectedMap = primary != null
      ? {...currentProfileVM2.b, GroupName.GLOBAL.name: primary}
      : currentProfileVM2.b;
