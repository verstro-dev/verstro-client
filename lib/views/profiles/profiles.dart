...
import 'package:fl_clash/verstro/verstro_flags.dart';
...
class _ProfilesViewState extends State<ProfilesView> {
  ...
  // Verstro 2.3.5: 隐藏手动导入入口. FAB 是进入"添加订阅"(URL/文件/扫码) 的唯一
  // 图形入口, 返 null 即斩断整条手动导入链 (AddProfileView/ScanPage 无从打开).
  // 订阅由 _verstroAutoIntegrate 自动 import; 手动刷新仍走顶栏 sync (_buildActions).
  Widget? _buildFAB() {
    if (kVerstroHideManualImport) return null;
      ...
      builder: (_, ref, _) {
        ...
        return CommonScaffold(
          ...
          body: state.profiles.isEmpty
              ? NullStatus(
                  // Verstro 2.3.5: 手动导入已隐藏, 原文案"请先添加配置"会让用户卡死
                  // (无入口可点). 正常流程 _verstroAutoIntegrate 已自动 import; 走到
                  // 这里说明自动 import 还没完成或失败 → 引导回账户页重试, 而非手动加.
                  label: kVerstroHideManualImport
                      ? appLocalizations.vAppProfilesSyncingTip
