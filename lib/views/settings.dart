...
// 分享推广入口: 海报 + 文案(带邀请码归因), 页面见 verstro/pages/share_page.dart。
class _ShareItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListItem(
      ...
      title: Text(context.appLocalizations.vAppShareTitle),
      subtitle: Text(context.appLocalizations.vAppShareSubtitle),
      onTap: () => Navigator.of(context).push(
        MaterialPageRoute(builder: (_) => const VerstroSharePage()),
      ),
    );
  }
}
...
class _HelpItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListItem(
      ...
      title: Text(context.appLocalizations.vHelpTitle),
      subtitle: Text(context.appLocalizations.vHelpIntro),
      onTap: () => openVerstroHelpCenter(context),
    );
  }
}
...
class _LogoutItem extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListItem(
      ...
      title: Text(
        context.appLocalizations.vAppLogout,
        style: TextStyle(color: error),
      ),
      onTap: () async {
        final confirm = await globalState.showMessage(
          title: context.appLocalizations.vAppLogout,
          message: TextSpan(text: context.appLocalizations.vAppLogoutConfirm),
        );
        if (confirm == true) {
