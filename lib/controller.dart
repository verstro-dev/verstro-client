...
import 'verstro/session_lifecycle_plan.dart';
...
extension InitControllerExt on AppController {
  ...
  Future<void> _initStatus() async {
    ...
    // autoRun 默认已关；hasProfile 仍防空启动死代理。Android 上若 VPN 已在跑，
    // globalState.isStart 为 true，保持挂回已有隧道，不因 autoRun=false 误停。
...
extension SystemControllerExt on AppController {
  ...
  Future<void> handleLogoutTeardown() async {
    final plan = planLogoutSession(isStarted: globalState.isStart);
    if (plan.exitApp) {
      await handleExit();
      return;
    }
