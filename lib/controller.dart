...
import 'verstro/verstro_flags.dart';
...
extension InitControllerExt on AppController {
  Future<void> _init() async {
    ...
    updateTray();
    autoUpdateProfiles();
    // 私有化托管服务: 不在启动时自动 ping GitHub release 检查更新 (FlClash 私有
    // 更新机制), 设置项也已移除。手动检查路径 (checkUpdateResultHandle isUser) 保留。
  ...
  Future<void> _initStatus() async {
    ...
    // Verstro 2.4 fix: autoRun 仅当已有 profile 时才自动连接.
    // 否则首次登录时 _verstroAutoIntegrate(application.dart, 在 attach 之后) 还没
    // import 订阅, autoRun 会让核心空启动 → isStart=true → 随后订阅 fetch 经
    // handleFindProxy 走 PROXY localhost:mixedPort (死代理端口) → connection refused.
    // 加 hasProfile guard 后: 首登无 profile → 不空启动 → fetch 走 DIRECT → 成功 import;
    // 返回用户已有 profile → autoRun 正常自连. (空启动核心本就无意义, 此 guard 普适正确)
...
extension StateControllerExt on AppController {
  ...
  SetupParams get setupParams {
    ...
    // 把 GLOBAL 钉到主线路选择器组：全局模式经 GLOBAL 委托给线路组路由，
    // 避免内核 GLOBAL selector 默认落 DIRECT。core 侧 patchSelectGroup 会 ForceSet。
    final primary = _ref.read(groupsProvider).primarySelectorName;
    if (primary != null) {
      selectedMap[GroupName.GLOBAL.name] = primary;
    }
...
extension SetupControllerExt on AppController {
  ...
  void changeMode(Mode mode) {
    ...
    if (mode == Mode.global) {
      // GLOBAL tab 已隐藏：激活 tab 落在可见的主线路选择器；并把 GLOBAL 实时钉到线路组，
      // 让切到全局即刻经线路组当前节点生效(而非内核默认 DIRECT)。
      final primary = _ref.read(groupsProvider).primarySelectorName;
      if (primary != null) {
        updateCurrentGroupName(primary);
        updateCurrentSelectedMap(GroupName.GLOBAL.name, primary);
        unawaited(changeProxy(groupName: GroupName.GLOBAL.name, proxyName: primary));
      }
    }
    addCheckIp();
  }
  ...
  Future<void> _setupConfig([VoidCallback? preloadInvoke]) async {
    ...
    // iOS/sing-box：核心跑在 VerstroTunnel NE 扩展里，配置经 App Group 下发，profile 内容
    // 本身就是完整 sing-box JSON（订阅用 sing-box UA 拉的）。不走桌面/安卓那套 mihomo
    // 解析(getConfig)+合并(makeRealProfileTask)+YAML 编码——它们对 sing-box 不适用，且 iOS
    // 的 getConfig 是 stub 会拿到 null。这里把原始 profile 内容喂给 coreExtension.config，
    // 起隧道时(startListener→start)随之下发到 App Group。
    // ⚠️ 必须调 preloadInvoke：桌面是在 coreController.setupConfig 末尾触发它
    // (= handleStart→startListener)，这里提前 return 跳过了那条路径，不补则隧道永不启动。
    ...
    if (system.isIOS) {
      ...
      coreExtension?.config =
          profileFile != null ? await profileFile.readAsString() : null;
      preloadInvoke?.call();
      addCheckIp();
      return;
    }
    ...
    if (enableTun != realTunEnable && realTunEnable == false) {
      ...
      switch (code) {
        ...
        case AuthorizeCode.error:
          enableTun = false;
          // 提权失败/取消时, 把持久化的 TUN 意图也落为「关」。否则 patchClashConfig.tun.enable
          // 仍为 true, 下次 applyProfile / updateConfig 会再次触发 authorizeCore; 当授权持续
          // 失败 (nosuid 挂载 / AppImage 无法 setuid / 覆盖安装清 setuid 后用户取消) 时,
          // 就会无限弹密码框、连主窗口都点不动。落为关后, 需用户主动重开 TUN 才再提权。
          _ref
              .read(patchClashConfigProvider.notifier)
              .update(
                (state) => state.tun.enable
                    ? state.copyWith.tun(enable: false)
                    : state,
              );
          break;
      }
...
extension SystemControllerExt on AppController {
  ...
  void initLink() {
    // Verstro 2.3.5: 不注册 clash://install-config 深链监听. 否则外部链接能绕过
    // 已隐藏的手动导入 UI, 往 Mihomo 注入非托管 profile (破坏自动托管订阅).
    // Verstro 订阅唯一来源 = _verstroAutoIntegrate 自动 import.
    if (kVerstroHideManualImport) return;
...
extension CommonControllerExt on AppController {
  ...
  void updateMode() {
    _ref.read(patchClashConfigProvider.notifier).update((state) {
      const modes = ModeExt.visibleValues;
      final index = modes.indexWhere((item) => item == state.mode);
      // 当前若是已隐藏的 direct(index==-1)或在末位，循环回到首个可见模式。
      final nextIndex = index + 1 > modes.length - 1 ? 0 : index + 1;
      return state.copyWith(mode: modes[nextIndex]);
    });
  }
