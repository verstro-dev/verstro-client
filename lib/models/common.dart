...
extension GroupsExt on List<Group> {
  ...
  /// 主线路选择器组名：可见(hidden==false)、非 GLOBAL 的 Selector 组；多个时取首个。
  /// 用于把 GLOBAL 钉到该组，实现全局模式经线路组委托路由。
  String? get primarySelectorName {
    for (final group in this) {
      if (group.hidden != false) continue; // 只看显式可见的组(与规则模式过滤一致)
