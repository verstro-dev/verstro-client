...
abstract class AppSettingProps with _$AppSettingProps {
  const factory AppSettingProps({
    ...
    // Verstro 2.4 配置硬化: 开机自启默认开 (VPN 常驻体验). TUN 默认开推迟 2.6
    // (需真签名 + 持久 helper, 见 application.dart _verstroAutoIntegrate 注释).
    @Default(true) bool autoLaunch,
    @Default(false) bool silentLaunch,
    // Verstro 2.4: 启动自动连接默认开. VerstroGate 确保进 Application 前已有有效订阅,
    // _verstroAutoIntegrate 先 import profile 再 init()→_initStatus 读 autoRun,
    // 故 autoRun 触发时必有可用 profile, 不会连到空配置.
