...
import 'subscription_gate_plan.dart';
...
class _VerstroGateState extends ConsumerState<VerstroGate> {
  ...
  void initState() {
    super.initState();
    // 桌面端原生窗口在 window.init 中创建为隐藏态, 只有 Application(controller._init)
    // 才会调 window.show(). VerstroGate 未登录/无订阅时停在 auth/PlanPicker, 永不进
    // Application → 窗口永不点亮. Windows/Linux 首发暴露此洞: 无窗口 / 进程活着占单实例
    // 锁 / 再点无反应 (macOS 原生窗口默认可见故未暴露). 首帧后主动 show, 与 controller
    // 一致尊重 silentLaunch; 登录后 Application 再 show 幂等无害.
    if (system.isDesktop) {
...
class _SubscriptionGate extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final view = planSubscriptionGate(
      hasData: sub != null,
      hasError: subAsync.hasError,
      isLoading: subAsync.isLoading,
      hasSubscription: sub?.hasSubscription ?? false,
      isExpired: sub?.isExpired ?? false,
    );
    switch (view) {
      case SubscriptionGateView.loading:
        return _BootSplash(message: appLocalizations.vAcctCheckingSubscription);
      case SubscriptionGateView.error:
        return _wrapTheme(
          Scaffold(
            appBar: AppBar(
              title: Text(appLocalizations.vAcctSubQueryFailedTitle),
              actions: [
                IconButton(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text('${subAsync.error}', textAlign: TextAlign.center),
                    const SizedBox(height: 12),
                    FilledButton.tonal(
      ...
      case SubscriptionGateView.planPicker:
        return _wrapTheme(const VerstroPlanPickerPage());
      case SubscriptionGateView.application:
        return child;
    }
  }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                Text(
                  appLocalizations.vAcctBootFailed,
