...
Future<void> main() async {
  ...
  // 独立入口不走 globalState.init, 需自行加载 l10n, 否则 appLocalizations 首次访问 assert 崩
...
class _LoginGateState extends ConsumerState<_LoginGate> {
  ...
  String _resetEmail = ''; // 找回密码: forgot 发码后带到 reset 页
  ...
  Widget build(BuildContext context) {
    ...
    return authAsync.when(
      ...
      error: (e, _) => Scaffold(
        body: Center(child: Text(appLocalizations.vAcctBootFailedWithError(e))),
      ),
      data: (state) {
        if (state.isLoggedIn) {
