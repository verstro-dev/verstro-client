...
import 'account_entitlement_models.dart';
...
Map<String, dynamic> buildCreateOrderBody(
  ...
  if (!const {'automatic', 'independent'}.contains(purchaseMode)) {
    throw ArgumentError.value(purchaseMode, 'purchaseMode');
  }
...
class BackendApi {
  ...
  }) async => EntitlementConversionQuote.fromJson(
    await _post(
      '/v1/entitlement-conversions/quote',
      {
        'target_tier': targetTier,
        'use_credit': useCredit,
        if (recoverFromOrderId > 0) 'recover_from_order_id': recoverFromOrderId,
        if (couponCode != null && couponCode.trim().isNotEmpty)
  ...
  Future<EntitlementConversion> createEntitlementConversion({
    ...
    required String idempotencyKey,
  }) async => EntitlementConversion.fromJson(
    await _post(
      '/v1/entitlement-conversions',
      {'request': request, 'quote': quote.toJson()},
  ...
  Future<EntitlementConversion> getEntitlementConversion(String id) async =>
      EntitlementConversion.fromJson(
        await _get(
          '/v1/entitlement-conversions/${Uri.encodeComponent(id)}',
          headers: const {'Cache-Control': 'no-store'},
        ),
      );
  ...
  Future<EntitlementConversion> requoteEntitlementConversion(
    ...
    await _post(
      '/v1/entitlement-conversions/${Uri.encodeComponent(id)}/requote',
      {'request': request},
      headers: const {'Cache-Control': 'no-store'},
    ),
  );
  ...
  Future<EntitlementConversion> confirmEntitlementConversion(
    ...
    await _post(
      '/v1/entitlement-conversions/${Uri.encodeComponent(id)}/confirm',
      {'revision': revision, 'quote_token': quoteToken},
      headers: const {'Cache-Control': 'no-store'},
    ),
  );

  Future<EntitlementConversion> cancelEntitlementConversion(
    String id, {
    required int revision,
  }) async => EntitlementConversion.fromJson(
    await _post(
      '/v1/entitlement-conversions/${Uri.encodeComponent(id)}/cancel',
      {'revision': revision},
      headers: const {'Cache-Control': 'no-store'},
    ),
  );
