...
class PendingOrderDto {
  ...
  const PendingOrderDto({required this.orderId, required this.receivedCents,
    required this.remainingCents, required this.expiresAt});
  ...
  factory PendingOrderDto.fromJson(Map<String, dynamic> json) {
    ...
    if (parsed == null) throw const FormatException('Invalid pending order expires_at');
...
class CreditDto {
  ...
  const CreditDto({required this.balanceCents, required this.credits,
    this.pendingOrderCents = 0, this.pendingOrders = const []});

  factory CreditDto.fromJson(Map<String, dynamic> json) {
    // 只有旧后端两个字段都缺省才兼容；新版损坏数据不能被当成零。
    var total = 0;
    var pending = <PendingOrderDto>[];
    if (json.containsKey('pending_order_cents') || json.containsKey('pending_orders')) {
      ...
      if (ids.length != pending.length ||
          pending.fold<int>(0, (sum, order) => sum + order.receivedCents) != rawTotal) {
    ...
    return CreditDto(
      ...
      credits: (json['credits'] as List<dynamic>?)
          ?.map((e) => CreditItemDto.fromJson(e as Map<String, dynamic>)).toList() ?? <CreditItemDto>[],
    );
  }
}
