...
/// 当前账号可购买套餐。依赖 auth，避免用户切换后沿用上一个账号的代理专属价。
final plansProvider = FutureProvider<List<PlanDto>>((ref) async {
  final auth = ref.watch(authNotifierProvider).value;
  if (auth == null || !auth.isLoggedIn) return <PlanDto>[];
...
/// - 网络错误不停, 下个 5s 再试
final orderDetailStreamProvider = StreamProvider.autoDispose
    .family<OrderDto, int>((ref, orderId) async* {
      final api = await ref.read(backendApiProvider.future);
      ...
      while (true) {
        try {
          ...
          // 错误必须传给 UI 隐藏过时付款指令，下一轮仍可恢复。
          yield* Stream<OrderDto>.error(e);
        }
...
Future<ClaimTxResult> claimTx(WidgetRef ref, int orderId, String txHash) async {
  final api = await ref.read(backendApiProvider.future);
  final result = await api.claimTx(orderId, txHash);
  // 部分到账也改变 pending；不在客户端计算或重复记账。
  if (ref.context.mounted) invalidateAccount(ref.invalidate);
  return result;
}
