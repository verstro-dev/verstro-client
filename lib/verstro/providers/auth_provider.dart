...
class AuthNotifier extends AsyncNotifier<AuthState> {
  ...
  Future<void> register({required String email, required String password, String? referralCode}) async {
    state = const AsyncValue.data(AuthState(loading: true));
    try {
      final api = await ref.read(backendApiProvider.future);
      final auth = await api.register(email: email, password: password, referralCode: referralCode)
      ...
      state = AsyncValue.data(AuthState(error: appLocalizations.vAuthRegisterFailed('$e')));
    }
  }
  ...
  Future<void> login({required String email, required String password}) async {
    ...
    try {
      ...
      // 兜底: timeout / 平台错误 / 等
      state = AsyncValue.data(AuthState(error: appLocalizations.vAuthLoginFailed('$e')));
    }
  }

  Future<void> logout() async {
    await appController.handleLogoutTeardown();
    final api = await ref.read(backendApiProvider.future);
    await api.logout();
  ...
  /// 重新拉 /me 刷新用户态 (邮箱验证成功后调, 让 isEmailVerified 更新). 失败保持原态.
