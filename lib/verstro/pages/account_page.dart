...
class VerstroAccountPage extends ConsumerWidget {
  ...
  Future<void> _refresh(WidgetRef ref) async {
    ...
    ].map((future) => future.then<dynamic>((value) => value, onError: (Object _, StackTrace _) => null)));
  }
...
class _MyPromotionsEntry extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final count =
        (data?.automatic.where((item) => item.availability == 'available').length ?? 0) +
    ...
    return Card(
      child: ListTile(
        ...
        onTap: () => Navigator.of(context).push(
          MaterialPageRoute<void>(builder: (_) => const VerstroMyPromotionsPage()),
        ),
      ),
    );
  }
...
class _MembershipCardEntry extends StatelessWidget {
  const _MembershipCardEntry();
  ...
  Widget build(BuildContext context) {
    return Card(
      child: ListTile(
        ...
        trailing: const Icon(Icons.chevron_right),
        onTap: () => Navigator.of(context).push(
          MaterialPageRoute<void>(builder: (_) => const MembershipCardsPage()),
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final subtitle = devicesAsync.maybeWhen(
      data: (info) => info.maxDevices > 0
          ? appLocalizations.vAcctDevicesRegistered(
              info.devices.length, info.maxDevices)
    ...
    return Card(
      child: ListTile(
        ...
        trailing: const Icon(Icons.chevron_right),
        onTap: () => Navigator.of(context).push(
          MaterialPageRoute(builder: (_) => const VerstroDevicesPage()),
        ),
      ),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.mark_email_unread_outlined, color: scheme.primary, size: 20),
                const SizedBox(width: 8),
                Text(appLocalizations.vAcctVerifyEmailTitle,
                    style: Theme.of(context).textTheme.titleMedium),
              ],
            ),
...
class _SubscriptionCard extends ConsumerWidget {
  ...
  // id→本地化名统一走 localizedPlanName (util/plan_display.dart); 无 id 时兜底 '-'.
  String _planLabel(String? id) => localizedPlanName(id ?? '', id ?? '-');

  // 活跃(未过期)套餐的去重名称, 保持 grants 的 FEFO 顺序(最早到期在前).
  // 多套餐并存时卡片标题不能只挂某一个套餐(原来挂 current_plan_id=最后购买的, 会和聚合的到期/上限自相矛盾).
  // 按 planId 走 localizedPlanName 本地化展示; 未知 id 回退后端下发的 planName (兼容旧后端/新套餐).
  List<String> _activePlanNames(SubscriptionDto sub) {
    final seen = <String>{};
    final names = <String>[];
    for (final g in sub.grants) {
      if (g.status == 'expired' || g.planName.isEmpty) continue;
      final name = localizedPlanName(g.planId, g.planName);
  ...
  String _remainingDays(DateTime? expiresAt) {
    ...
    if (diff.isNegative) return appLocalizations.vAcctExpired;
    if (diff.inDays >= 1) return appLocalizations.vAcctDaysLater(diff.inDays);
    if (diff.inHours >= 1) {
      return appLocalizations.vAcctHoursLater(diff.inHours);
    }
    return appLocalizations.vAcctMinutesLater(diff.inMinutes);
  }
    ...
    return subAsync.when(
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(appLocalizations.vAcctSubQueryFailed,
      ...
      data: (sub) {
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text(appLocalizations.vAcctNoSubscription,
                          style: Theme.of(context).textTheme.titleMedium),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Text(
                    appLocalizations.vAcctNoSubscriptionDesc,
                  ...
                  FilledButton.icon(
                    ...
                    label: Text(appLocalizations.vAcctBuyPlan),
        ...
        final badgeLabel = planNames.length >= 2
            ? appLocalizations.vAcctMultiPlanBadge
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(
                      expired
                          ? appLocalizations.vAcctSubExpired
                          : appLocalizations.vAcctSubActive,
                      style: Theme.of(context).textTheme.titleMedium,
                    ),
                    const Spacer(),
                ...
                const SizedBox(height: 12),
                _InfoRow(label: appLocalizations.vAcctPlanLabel, value: planValue),
                _InfoRow(
                  label: appLocalizations.expirationTime,
                  value: sub.periodExpiresAt != null
                      ? sub.periodExpiresAt!.toLocal().toString().split('.')[0]
                      : '-',
                ),
                _InfoRow(
                  label: appLocalizations.vAcctRemainingLabel,
                  value: _remainingDays(sub.periodExpiresAt),
                  valueColor: expired ? scheme.error : null,
                ),
                _InfoRow(
                  label: appLocalizations.vAcctTrafficLimitLabel,
                  value: _formatBytes(sub.trafficLimitBytes),
                ),
                if (sub.grants.isNotEmpty)
                  _InfoRow(
                    label: appLocalizations.vAcctTotalRemainingLabel,
                ...
                const SizedBox(height: 16),
                // 订阅链接: 可复制到 Shadowrocket 等第三方客户端 (尤其 iOS 无原生 app 时).
                // 仅在有效未过期时展示 —— 订阅链接是 bearer 凭证, 过期凭证不必再摆出制造泄露面.
                if (sub.subscriptionUrl != null && !expired) ...[
                  Container(
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            const Spacer(),
                            IconButton(
                              tooltip: appLocalizations.vAcctCopySubscriptionUrl,
                              visualDensity: VisualDensity.compact,
                              icon: const Icon(Icons.copy, size: 18),
                              onPressed: () {
                                Clipboard.setData(
                                    ClipboardData(text: sub.subscriptionUrl!));
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(appLocalizations
                                        .vAcctSubscriptionUrlCopied),
                        ...
                        const SizedBox(height: 6),
                        Text(
                          appLocalizations.vAcctSubscriptionUrlDesc,
                          style: Theme.of(context).textTheme.bodySmall?.copyWith(
                                color: scheme.onSurfaceVariant,
                              ),
                        ),
                ...
                FilledButton.tonalIcon(
                  ...
                  label: Text(expired
                      ? appLocalizations.vAcctRepurchase
                      : appLocalizations.vAcctRenewUpgrade),
                  onPressed: () => _goRenew(context),
                ),
              ],
            ),
      ...
      error: (e, _) => Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 8),
            Text(appLocalizations.vAcctOrdersQueryFailed(e),
...
class _OrderTile extends StatelessWidget {
  ...
  ({String label, Color color, IconData icon}) _statusBadge(BuildContext ctx) {
    ...
    switch (order.status) {
      case 'finished':
        return (label: appLocalizations.vAcctOrderPaid, color: Colors.green.shade400, icon: Icons.check_circle);
      case 'waiting':
        final exp = order.expiresAt.isBefore(DateTime.now());
        if (exp) {
          return (label: appLocalizations.vAcctExpired, color: scheme.error, icon: Icons.access_time_filled);
        }
        return (label: appLocalizations.vAcctOrderWaiting, color: scheme.primary, icon: Icons.hourglass_top);
      case 'partially_paid':
        final exp = order.expiresAt.isBefore(DateTime.now());
        if (exp) {
          return (label: appLocalizations.vAcctExpired, color: scheme.error, icon: Icons.access_time_filled);
      ...
      case 'expired':
        return (label: appLocalizations.vAcctExpired, color: scheme.error, icon: Icons.access_time_filled);
      case 'failed':
        return (label: appLocalizations.vAcctOrderFailed, color: scheme.error, icon: Icons.cancel);
      default:
    ...
    return devicesAsync.when(
      ...
      error: (e, _) => Card(
        child: ListTile(
          ...
          subtitle: Text(appLocalizations.vAcctTryLater,
              style: Theme.of(context).textTheme.bodySmall),
          trailing: TextButton(
          ...
          children: [
            ...
            if (info.maxDevices > 0)
              Padding(
                ...
                child: Row(
                  children: [
                    Icon(Icons.devices, size: 14, color: scheme.onSurfaceVariant),
                    const SizedBox(width: 4),
                    Text(
                      appLocalizations.vAcctDevicesRegistered(
                          devices.length, info.maxDevices),
                      style: Theme.of(context).textTheme.bodySmall?.copyWith(
                            color: scheme.onSurfaceVariant,
                          ),
                    ),
          ...
          children: [
            ...
            if (isCurrent) ...[
              ...
              Container(
                ...
                child: Text(appLocalizations.vAcctThisDevice,
                    style: TextStyle(
      ...
      children: [
        Row(
          children: [
            ...
            const SizedBox(width: 4),
            Text(
              appLocalizations.vAcctTrafficUsage,
              style: Theme.of(context).textTheme.bodySmall?.copyWith(
                    color: scheme.onSurfaceVariant,
                  ),
            ),
        ...
        if (nearLimit) ...[
          const SizedBox(height: 4),
          Text(
            appLocalizations.vAcctTrafficNearLimit,
            style: Theme.of(context).textTheme.bodySmall?.copyWith(
                  fontSize: 11,
                  color: scheme.error,
                ),
      ...
      children: [
        Row(
          children: [
            ...
            const SizedBox(width: 4),
            Text(
              appLocalizations.vAcctPlanDetails,
...
class _GrantTile extends StatelessWidget {
  ...
  String _statusLabel(String s) {
    switch (s) {
      case 'active':
        return appLocalizations.vAcctGrantActive;
      case 'exhausted':
        return appLocalizations.vAcctGrantExhausted;
      case 'expired':
        return appLocalizations.vAcctExpired;
      default:
        return s;
    }
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Text(
                localizedPlanName(grant.planId, grant.planName),
                style: Theme.of(context).textTheme.bodyMedium?.copyWith(
              ...
              Text(
                appLocalizations
                    .vAcctRemainingBytes(_formatBytes(grant.remainingBytes)),
                style: const TextStyle(
                  fontFamily: 'monospace',
                  fontSize: 12,
                  fontWeight: FontWeight.w500,
          ...
          Row(
            children: [
              ...
              if (expiry.isNotEmpty)
                Text(
                  appLocalizations.vAcctExpiresOn(expiry),
                  style: Theme.of(context).textTheme.bodySmall?.copyWith(
                        color: scheme.onSurfaceVariant,
                      ),
                ),
