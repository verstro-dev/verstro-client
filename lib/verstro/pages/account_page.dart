  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final subtitle = devicesAsync.maybeWhen(
      data: (info) => info.maxDevices > 0
          ? '已登记 ${info.devices.length} / ${info.maxDevices} 台'
          : '已登记 ${info.devices.length} 台',
      orElse: () => '管理已登录的设备',
    );
    return Card(
      child: ListTile(
          ...
          children: [
            Row(
              children: [
                ...
                Text('验证邮箱', style: Theme.of(context).textTheme.titleMedium),
              ],
            ),
            const SizedBox(height: 6),
            Text(
              '验证码已发送到 $email（含垃圾箱）。输入 6 位验证码完成验证。',
              style: Theme.of(context).textTheme.bodySmall?.copyWith(
  ...
  ({String label, Color color, IconData icon}) _statusBadge(BuildContext ctx) {
    ...
    switch (order.status) {
      ...
      case 'waiting':
        ...
        if (exp) {
          return (label: '已过期', color: scheme.error, icon: Icons.access_time_filled);
        }
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 4),
                    Text(
                      '已登记 ${devices.length} / ${info.maxDevices} 台',
                      style: Theme.of(context).textTheme.bodySmall?.copyWith(
                            color: scheme.onSurfaceVariant,
                          ),
                    ),
