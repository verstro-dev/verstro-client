...
class MembershipCardEntry extends ConsumerWidget {
  const MembershipCardEntry({super.key});
...
class OrderHistoryEntry extends ConsumerWidget {
  const OrderHistoryEntry({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Card(
      child: ListTile(
        ...
        title: Text(appLocalizations.vAcctOrdersEntitlements),
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: TabBar(
            tabs: [
              Tab(text: appLocalizations.vAcctOrderTab),
              Tab(text: appLocalizations.vAcctEntitlementTab),
            ],
          ),
        ),
        body: TabBarView(
          children: [
            ...
            RefreshIndicator(
              ...
              child: ListView(
                padding: const EdgeInsets.all(16),
                children: const [EntitlementHistoryList()],
              ),
            ),
          ],
        ),
...
class _OrderTile extends StatelessWidget {
  ...
  String _payAmountLabel() {
    final credit = order.creditApplied;
    final creditCents = credit == null || credit.isEmpty
        ? 0
        : usdToCents(credit);
    final newPayCents = order.isPartiallyPaid
        ? order.remainingCents
        : usdToCents(order.finalAmount);
    final split = orderPaymentSplit(
      creditAppliedCents: creditCents,
      newPayCents: newPayCents,
    );
    if (!split.hasCredit) {
      return '\$${order.finalAmount} USDT';
    }
    return '${appLocalizations.vAcctOrderPayCredit} \$${centsToUsd(split.creditCents)} · ${appLocalizations.vAcctOrderPayNew} \$${centsToUsd(split.newPayCents)}';
  }
