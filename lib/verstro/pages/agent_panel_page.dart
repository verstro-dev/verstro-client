...
class VerstroAgentPanelPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<VerstroAgentPanelPage> createState() => _AgentPanelPageState();
}
...
class _AgentPanelPageState extends ConsumerState<VerstroAgentPanelPage> {
          ...
          actions: [
            TextButton(
              onPressed: () {
                FocusScope.of(
                  ctx,
                ).unfocus(); // 关弹窗前释放焦点, 避免 TextField 焦点 teardown 崩溃
                Navigator.pop(ctx);
              },
            ...
            TextButton(
              onPressed: () {
                if (!_isTrc20(_payoutAddrCtrl.text)) {
                  setLocal(
                    () => err = appLocalizations.vAgentPayoutAddrInvalid,
                  );
                  return;
                }
  ...
  Future<void> _setPrice(AgentPlanPriceDto p) async {
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setLocal) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                appLocalizations.vAgentPriceRangeHint(
                  _usd(p.floorCents),
                  _usd(p.listCents),
                ),
              ),
            ...
            TextButton(
              onPressed: () {
                ...
                if (cents < p.floorCents || cents > p.listCents) {
                  setLocal(
                    () => err = appLocalizations.vAgentPriceOutOfRange(
                      _usd(p.floorCents),
                      _usd(p.listCents),
                    ),
                  );
    ...
    try {
      ...
      _toast(
        appLocalizations.vAgentPriceSetSuccess(
          p.planId,
  ...
  Widget _buildBody(BuildContext context, AgentDto a) {
    ...
    // 分享链接携带邀请码并直达注册页；注册成功后归因进入账号，跨 App/网站共用。
    final shareText = appLocalizations.vAgentShareText(
      a.code,
      buildPartnerRegistrationUrl(a.code),
    );
    final partner = a.partnerAuthorization;
    final partnerLabel = _partnerLabel(partner);
      ...
      children: [
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    const Spacer(),
                    Chip(
                      label: Text(partnerLabel ?? _tierLabel(a.tier)),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                appLocalizations.vPartnerVerified,
                              ...
                              Text(
                                appLocalizations.vPartnerAuthorizationCode(
                                  partner!.authorizationCode,
                                ),
                              ),
                              if (partner.isNonExclusive)
                                Text(
                                  appLocalizations.vPartnerNonExclusive,
                ...
                Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () => _copy(
                        a.code,
                        appLocalizations.vAgentInviteCodeCopied,
                      ),
                    ),
                  ],
                ),
                ...
                Wrap(
                  ...
                  children: [
                    OutlinedButton.icon(
                      ...
                      onPressed: () => _copy(
                        shareText,
                        appLocalizations.vAgentShareTextCopied,
                      ),
                    ),
                    OutlinedButton.icon(
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  appLocalizations.vAgentWalletTitle,
                  style: Theme.of(context).textTheme.titleMedium,
                ),
                const SizedBox(height: 12),
                // 可提现金额 — 主色大字
                ...
                Text(
                  appLocalizations.vAgentPending(_usd(a.pendingCents)),
                  style: Theme.of(context).textTheme.bodyMedium,
                ),
                if (a.processingCents > 0)
                  Text(
                    appLocalizations.vAgentProcessing(_usd(a.processingCents)),
                    style: Theme.of(context).textTheme.bodyMedium,
                  ),
                Text(
                  appLocalizations.vAgentPaid(_usd(a.paidCents)),
                  style: Theme.of(context).textTheme.bodyMedium,
                ),
                const SizedBox(height: 4),
                Text(
                  appLocalizations.vAgentInvitedCount(a.directCount),
                  style: Theme.of(context).textTheme.bodySmall,
                ),
        ...
        if (a.processingCents > 0) ...[
          ...
          SizedBox(
            width: double.infinity,
            child: FilledButton(
              onPressed: _payoutBusy
                  ? null
                  : () => _payout(a.availableCents, minCents),
              child: _payoutBusy
                  ? const SizedBox(
                      height: 18,
                      width: 18,
        ...
        if (a.payouts.isNotEmpty) ...[
          ...
          Text(
            appLocalizations.vAgentPayoutHistory,
            style: Theme.of(context).textTheme.titleMedium,
          ),
          const SizedBox(height: 8),
        ...
        if (partner?.isVerified == true &&
            partner!.hasCapability('custom_pricing')) ...[
          const SizedBox(height: 20),
          Text(
            appLocalizations.vAgentPlanPricing,
            style: Theme.of(context).textTheme.titleMedium,
          ),
          const SizedBox(height: 8),
          ref
              .watch(agentPricesProvider)
              .maybeWhen(
                data: (prices) => Column(
                  children: prices.prices
                      .map((p) => _buildPriceCard(context, p))
                      .toList(),
                ),
  ...
  Widget _buildPriceCard(BuildContext context, AgentPlanPriceDto p) {
    final wholesale = p.wholesaleCents > 0 ? p.wholesaleCents : p.floorCents;
    final earns = p.customCents != null ? p.customCents! - wholesale : null;
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              appLocalizations.vAgentMinimumSaleLine(_usd(p.floorCents)),
              style: Theme.of(context).textTheme.bodySmall,
            ),
            const SizedBox(height: 4),
            Text(
              ...
              Row(
                children: [
                  ...
                  TextButton(
                    ...
                    child: Text(
                      appLocalizations.vAgentViewOnTronScan,
