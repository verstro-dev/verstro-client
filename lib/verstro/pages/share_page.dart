...
enum ShareCopyStyle {
  ...
  final String label;
  const ShareCopyStyle(this.label);
}
...
String _inviteSentence({
  ...
  final prefix = brief ? '注册填邀请码 $code' : '注册时填我的邀请码 $code';
  if (refereeRewardCents > 0 && refereeRewardCents == referrerRewardCents) {
    return '$prefix，首购后你我各得 ${fmtUsdShort(refereeRewardCents)} 抵扣。';
  }
  if (refereeRewardCents > 0) {
    return '$prefix，首购后可得 ${fmtUsdShort(refereeRewardCents)} 抵扣。';
  }
  return '$prefix。';
}
  ...
  switch (style) {
    case ShareCopyStyle.general:
      final trial = trialEnabled
          ? '新用户可免费试用${trialDays > 0 ? ' $trialDays 天' : ''}。'
          : '';
      return '我在用 Verstro —— 隐私优先的全球网络加速。'
          '一键全局加速，手机电脑全平台，稳定不掉线，客户端开源可审计。'
          '$trial$invite下载：$kVerstroSiteUrl';
    case ShareCopyStyle.brief:
      final trial = trialEnabled ? '可免费试用。' : '';
      return 'Verstro 全球网络加速：全平台一键直连，稳定不掉线。'
          '$trial$invite$kVerstroSiteUrl';
    case ShareCopyStyle.developer:
      final trial = trialEnabled ? '可免费试用。' : '';
      return '受够了 brew / docker / git 不走代理？'
          'Verstro 用 TUN 从系统底层接管全部流量，命令行、桌面 App 全覆盖，'
          '多地节点智能切换，客户端 GPLv3 开源可审计。'
          '$trial$invite$kVerstroSiteUrl';
  }
}
...
class _SharePageState extends ConsumerState<VerstroSharePage> {
  ...
  Future<void> _savePoster() async {
    ...
    try {
      ...
      final path =
          await picker.saveFile('Verstro-邀请海报.png', data.buffer.asUint8List());
      _toast(path == null ? '已取消保存' : '海报已保存');
    } catch (_) {
      _toast('保存失败，请重试');
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('分享 Verstro')),
      body: ListView(
          ...
          if (agent.hasError) ...[
            ...
            Row(
              ...
              children: [
                ...
                const Text('邀请码加载失败', style: TextStyle(fontSize: 12)),
                TextButton(
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text('分享文案', style: Theme.of(context).textTheme.titleMedium),
...
class SharePoster extends StatelessWidget {
  ...
  String get _rewardLine {
    if (refereeRewardCents > 0 && refereeRewardCents == referrerRewardCents) {
      return '填码注册 · 首购后你我各得 ${fmtUsdShort(refereeRewardCents)}';
    }
    if (refereeRewardCents > 0) {
      return '填码注册 · 首购得 ${fmtUsdShort(refereeRewardCents)} 抵扣';
    }
    return '注册时填写此邀请码';
  }

  String get _trialLine {
    final days = trialDays > 0 ? '免费试用 $trialDays 天' : '支持免费试用';
      ...
      children: [
        ...
        const Column(
          ...
          children: [
            ...
            Text(
              '隐私优先的全球网络',
              style: TextStyle(color: _mist, fontSize: 11, height: 1.1),
            ),
          ],
        ),
      ],
  ...
  Widget _meridianSection() {
    const features = [
      ('真·全局代理', '所有 App 都能用，不只浏览器'),
      ('多地节点', '智能切换，稳定不掉线'),
      ('隐私优先', '不囤日志，加密不留痕'),
      ('全平台', 'Android · iOS · Windows · macOS'),
    ];
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Padding(
                    ...
                    child: Text(
                      '所有 App · 全程稳连不掉线',
                      style: TextStyle(color: _mist, fontSize: 13.5, height: 1.2),
                    ),
                  ),
                  const SizedBox(height: 16),
  ...
  Widget _invitePanel() {
    return Column(
      ...
      children: [
        const Text(
          '邀请码',
          style: TextStyle(color: _mist, fontSize: 10, letterSpacing: 3),
        ),
        ...
        const Text.rich(
          TextSpan(
            children: [
              TextSpan(
                text: '扫码下载 · ',
                style: TextStyle(color: _mist, fontSize: 10.5),
              ),
              TextSpan(
                text: 'verstro.com',
                style: TextStyle(
  ...
  Widget _downloadPanel() {
    return const Column(
      ...
      children: [
        ...
        Text(
          '扫码访问官网，下载客户端',
          style: TextStyle(color: _mist, fontSize: 11.5),
        ),
        SizedBox(height: 8),
        Text(
          'verstro.com',
