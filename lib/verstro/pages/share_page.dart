...
/// 代理分享统一落到可自动归因的网页注册页；无码时保留官网降级。
String buildPartnerRegistrationUrl(String code) {
  final normalized = code.trim().toUpperCase();
  if (normalized.isEmpty) return kVerstroSiteUrl;
  return Uri(
    scheme: 'https',
    host: 'verstro.com',
    path: '/account',
    queryParameters: {'ref': normalized},
    fragment: 'register',
  ).toString();
}
...
String _inviteSentence({
  ...
  if (refereeRewardCents > 0 && refereeRewardCents == referrerRewardCents) {
    return appLocalizations.vShareInviteBoth(
      prefix,
      fmtUsdShort(refereeRewardCents),
    );
  }
...
String buildShareText({
  ...
  final registrationUrl = buildPartnerRegistrationUrl(code);
  switch (style) {
    case ShareCopyStyle.general:
      final trial = trialEnabled
          ...
          : '';
      return appLocalizations.vShareCopyGeneral(trial, invite, registrationUrl);
    case ShareCopyStyle.brief:
      final trial = trialEnabled ? appLocalizations.vShareTrialShort : '';
      return appLocalizations.vShareCopyBrief(trial, invite, registrationUrl);
    case ShareCopyStyle.developer:
      final trial = trialEnabled ? appLocalizations.vShareTrialShort : '';
      return appLocalizations.vShareCopyDev(trial, invite, registrationUrl);
  }
}
  ...
  Future<void> _savePoster() async {
    ...
    try {
      ...
      final path = await picker.saveFile(
        appLocalizations.vSharePosterFileName,
          ...
          if (agent.hasError) ...[
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  appLocalizations.vShareCodeLoadFailed,
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    appLocalizations.vShareCopyTitle,
...
class SharePoster extends StatelessWidget {
  ...
  String get _rewardLine {
    if (refereeRewardCents > 0 && refereeRewardCents == referrerRewardCents) {
      return appLocalizations.vSharePosterRewardBoth(
        fmtUsdShort(refereeRewardCents),
      );
    }
    if (refereeRewardCents > 0) {
      return appLocalizations.vSharePosterRewardReferee(
        fmtUsdShort(refereeRewardCents),
      );
    }
  ...
  Widget _meridianSection() {
    final features = [
      ...
      (
        appLocalizations.vSharePosterFeat4Label,
      ...
      child: Row(
        children: [
          Container(
            ...
            child: QrImageView(
              data: buildPartnerRegistrationUrl(code),
          ...
          Expanded(child: code.isNotEmpty ? _invitePanel() : _downloadPanel()),
        ],
      ),
    );
  }
