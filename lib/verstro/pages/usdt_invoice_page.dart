...
class _VerstroUsdtInvoicePageState
                    ...
                    onPressed: busy || retrySeconds > 0
                        ...
                        : () async {
                            ...
                            try {
                              ...
                              if (d.kind == ClaimKind.success && ctx.mounted) {
                                if (mounted) {
                                  setState(() => _claimSuccessNotice = d.text);
                                }
  ...
  Widget _buildContent(BuildContext context) {
    ...
    ref.listen(orderDetailStreamProvider(widget.order.id), (previous, next) {
      ...
      if (after != null && (before?.status != after.status ||
    ...
    if (currentOrder.isExpired || !currentOrder.expiresAt.isAfter(widget.now())) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          if (hasDiscount) ...[
            _AmountBreakdownRow(
              label: appLocalizations.vPayBasePriceLabel,
              value: '\$$basePrice',
            ),
...
class _ExpiredScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text((serverConfirmed ? appLocalizations.vPayOrderExpiredTitle : appLocalizations.vPayLocalExpiryTitle))),
      body: Center(
        child: Padding(
          padding: const EdgeInsets.all(24),
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text(
                (serverConfirmed ? appLocalizations.vPayOrderExpiredTitle : appLocalizations.vPayLocalExpiryTitle),
                style: Theme.of(context).textTheme.headlineSmall,
              ),
              const SizedBox(height: 8),
              Text(
                (serverConfirmed ? appLocalizations.vPayOrderExpiredDesc(order.id) : appLocalizations.vPayLocalExpiryDesc),
                style: Theme.of(context).textTheme.bodyMedium,
                textAlign: TextAlign.center,
              ),
              const SizedBox(height: 16),
              ...
              FilledButton(
                ...
                child: Text((serverConfirmed ? appLocalizations.vPayBackToReorder : appLocalizations.vPayBackToHome)),
              ),
            ],
          ),
        ),
