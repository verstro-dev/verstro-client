          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '提交后 backend 会立即查链上验证. 多付会自动开通并把多付部分\n'
                  '存入账户余额; 少付则到账金额全额存入余额, 重新下单时自动抵扣.',
                    ...
                    onPressed: busy
                        ...
                        : () async {
                            ...
                            try {
                              ...
                              // resolution 分支 (新契约); 老后端无 resolution → 按 matched 布尔回退
                              final r = res.resolution;
                              final credited = r == 'credited_underpay' ||
                                  r == 'credited_expired';
                              final success = r == 'matched' ||
                                  r == 'overpaid_matched' ||
                                  (r == null && res.matched);
                              setLocalState(() {
                                busy = false;
                                if (credited) {
                                  creditedMsg = res.message;
                                } else if (success) {
                                  successMsg = res.message;
                                } else {
                                  // matched_other_order / already_processed /
                                  // rejected_manual / 未知枚举: 红字常驻后端 message
                                  error = res.message;
                                }
                              });
                              if (success) {
                  ...
                  OutlinedButton.icon(
                    onPressed: () async {
                      ...
                      if (await canLaunchUrl(uri)) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Telegram 未安装. 客服: @VerstroSupportBot'),
                          ),
                        );
                      }
                    },
                    icon: const Icon(Icons.support_agent),
                    label: const Text('联系人工客服 Bot (金额错配 / 转错地址)'),
                  ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            '交易所提币会额外收取网络手续费（TRC20 常见约 1 USDT）并从提币数量中'
            '扣除，实际到账会比填写的少，导致无法自动匹配。推荐用自己的钱包直接转账'
            '（imToken / TronLink）；必须用交易所时，提币数量 = 应付金额 + 手续费，'
            '确保「到账数量」恰好等于应付金额。多付会自动开通并把多付部分存入账户'
            '余额；少付则到账金额全额存入余额，重新下单时自动抵扣。',
            style: Theme.of(context).textTheme.bodySmall,
          ),
        ],
      ),
