...
// final_amount = base + (order_id * 7) % 100 cents。拆分付款由后端按整数
// cents 累计；客户端只展示结构化事实，不根据 message 猜测状态。
...
class VerstroUsdtInvoicePage extends ConsumerStatefulWidget {
  ...
  final bool conversionPayment;
  final DateTime Function() now;

  const VerstroUsdtInvoicePage({
    super.key,
    required this.order,
    this.conversionPayment = false,
    this.now = _systemNow,
  });

  @override
  ConsumerState<VerstroUsdtInvoicePage> createState() =>
      _VerstroUsdtInvoicePageState();
}

class _VerstroUsdtInvoicePageState
    extends ConsumerState<VerstroUsdtInvoicePage> {
  Timer? _countdownTimer;
  Timer? _claimRetryTimer;
  DateTime? _claimRetryDeadline;
  Duration _remaining = Duration.zero;
  String? _claimSuccessNotice;
  int? _invoiceOwnerId;
  ...
  void _updateRemaining() {
    final remaining = widget.order.expiresAt.difference(widget.now());
    if (mounted) {
      setState(
  ...
  int _claimRetrySecondsRemaining() {
    final deadline = _claimRetryDeadline;
    if (deadline == null) return 0;
    final remainingMs = deadline.difference(widget.now()).inMilliseconds;
    if (remainingMs <= 0) {
      _claimRetryDeadline = null;
      return 0;
    }
    return (remainingMs + Duration.millisecondsPerSecond - 1) ~/
        Duration.millisecondsPerSecond;
  }

  Future<void> _showClaimDialog(int orderId) async {
    Key hashFieldKey = UniqueKey();
    String hashInput = '';
    bool busy = false;
    bool continueSubmission = false;
    int retrySeconds = _claimRetrySecondsRemaining();
    bool retryTimerStarted = false;
    String? error;
    String? infoMsg;
    ...
    void startRetryCountdown(
      ...
      if (seconds != null) {
        _claimRetryDeadline = seconds > 0
            ? widget.now().add(Duration(seconds: seconds))
            : null;
      }
      _claimRetryTimer?.cancel();
      retrySeconds = _claimRetrySecondsRemaining();
      if (retrySeconds == 0) return;
      _claimRetryTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
        if (!mounted) {
          timer.cancel();
          return;
        }
        ...
        if (retrySeconds == 0) {
          _claimRetryDeadline = null;
          timer.cancel();
          if (identical(_claimRetryTimer, timer)) {
    ...
    final reorder = await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setLocalState) {
          if (!retryTimerStarted && retrySeconds > 0) {
            retryTimerStarted = true;
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (ctx.mounted) startRetryCountdown(ctx, setLocalState);
            });
          }
          return AlertDialog(
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  Text(appLocalizations.vPayClaimInstruction),
                  const SizedBox(height: 12),
                  TextField(
                    key: hashFieldKey,
                    onChanged: (value) => hashInput = value,
            ...
            actions: creditedMsg != null
                ...
                : [
                    ...
                    FilledButton(
                      onPressed: busy || retrySeconds > 0
                          ? null
                          : () async {
                              final hash = hashInput.trim();
                              if (hash.length < 32) {
                                setLocalState(
                                  () => error =
                                      appLocalizations.vPayTxHashLengthError,
                                );
                                return;
                              }
                              setLocalState(() {
                              ...
                              try {
                                ...
                                if (d.refreshOrder) {
                                  ref.invalidate(
                                    orderDetailStreamProvider(orderId),
                                  );
                                ...
                                setLocalState(() {
                                  busy = false;
                                  continueSubmission = d.continueSubmission;
                                  if (d.kind == ClaimKind.credited) {
                                    creditedMsg = d.text;
                                  } else if (d.kind == ClaimKind.partial ||
                                      d.kind == ClaimKind.pending) {
                                    infoMsg = d.text;
                                    if (d.kind == ClaimKind.partial) {
                                      hashInput = '';
                                      hashFieldKey = UniqueKey();
                                    }
                                  } else if (d.kind != ClaimKind.success) {
                                    error = d.text;
                                  }
                                });
                                if (d.retryAfterSeconds > 0) {
                                  startRetryCountdown(
                                    ctx,
                                    setLocalState,
                                    seconds: d.retryAfterSeconds,
                                  );
                                }
                                if (d.kind == ClaimKind.success &&
                                    ctx.mounted) {
                                  if (mounted) {
                                    setState(
                                      () => _claimSuccessNotice = d.text,
                                    );
                                  }
                                ...
                                setLocalState(() {
                                  busy = false;
                                  error = appLocalizations.vPaySubmitFailed(
                                    '$e',
                                  );
                                });
                              }
                      ...
                      child: busy
                          ...
                          : Text(
                              retrySeconds > 0
                                  ? appLocalizations.vPayRetryInSeconds(
                                      retrySeconds,
                                    )
                                  : continueSubmission
                                  ? appLocalizations.vPayContinuePaymentWithHash
                                  : appLocalizations.vPaySubmitVerify,
                            ),
                    ),
                  ],
          );
  ...
  Widget _buildContent(BuildContext context) {
    if (widget.order.isConversion) {
      final auth = ref.watch(authNotifierProvider);
      final uid = !auth.isLoading && !auth.hasError
          ? auth.value?.user?.id
          : null;
      _invoiceOwnerId ??= uid;
      if (uid == null || uid != _invoiceOwnerId) {
        return Scaffold(
          body: Center(child: Text(appLocalizations.vConvUnknown)),
        );
      }
    }
    // 监听订单状态变化 (5s 轮询)
    ref.listen(orderDetailStreamProvider(widget.order.id), (previous, next) {
      final before = previous?.value;
      final after = next.value;
      if (after != null &&
          after.hasSamePaymentIdentity(widget.order) &&
          (before?.status != after.status ||
              before?.receivedCents != after.receivedCents)) {
        invalidateAccount(ref.invalidate);
        ref.invalidate(ordersListProvider);
      }
    });
    final streamAsync = ref.watch(orderDetailStreamProvider(widget.order.id));
    // 缓存订单可能已被另一笔购买关闭；核验前或核验失败不得展示旧金额与二维码。
    if (streamAsync.isLoading ||
        streamAsync.hasError ||
        !streamAsync.hasValue) {
      return Scaffold(
        appBar: AppBar(
          title: Text(
            appLocalizations.vPayOrderTitle(
              _planLabel(widget.order.planId),
              widget.order.id,
            ),
          ),
        ),
        body: Center(
          child: streamAsync.hasError
              ? TextButton(
                  onPressed: () => ref.invalidate(
                    orderDetailStreamProvider(widget.order.id),
                  ),
                  child: Text(appLocalizations.vAcctCreditError),
                )
              : const CircularProgressIndicator(),
        ),
      );
    }
    final currentOrder = streamAsync.requireValue;
    if (!currentOrder.hasSamePaymentIdentity(widget.order)) {
      return Scaffold(
        body: Center(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text(appLocalizations.vConvUnknown),
              TextButton(
                onPressed: () =>
                    ref.invalidate(orderDetailStreamProvider(widget.order.id)),
    ...
    _remaining = currentOrder.expiresAt.difference(widget.now());

    // 旧付款单入口也回到具名换购，足额不跳普通激活成功屏。
    if (currentOrder.isConversion &&
        (!widget.conversionPayment ||
            currentOrder.isFinished ||
            (!currentOrder.isExpired && currentOrder.remainingCents <= 0))) {
      final id = currentOrder.conversionId;
      if (id == null || id.isEmpty) {
        return Scaffold(
          body: Center(child: Text(appLocalizations.vConvUnknown)),
        );
      }
      return EntitlementConversionPage(conversionId: id);
    }
    // 如果已 finished 或 expired, 显示终态屏
    if (currentOrder.isFinished) {
      return _SuccessScreen(
        order: currentOrder,
        claimNotice: _claimSuccessNotice,
      );
    }
    // 本地到期只关闭付款展示，不修改服务端状态，也不取消轮询。
    if (currentOrder.isExpired ||
        !currentOrder.expiresAt.isAfter(widget.now())) {
      // expired 订单现在也可以 claim (少付/过期到账自动入余额), 弹同一个 claim 弹窗
      return _ExpiredScreen(
        serverConfirmed: currentOrder.isExpired,
        order: currentOrder,
        onClaim: () => _showClaimDialog(currentOrder.id),
      );
    }

    final paymentAmount = currentOrder.isPartiallyPaid
        ? centsToUsd(currentOrder.remainingCents)
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          appLocalizations.vPayOrderTitle(
            _planLabel(currentOrder.planId),
            currentOrder.id,
          ),
        ),
        actions: [
              ...
              child: Column(
                ...
                children: [
                  ...
                  _AmountCard(
                    ...
                    isConversion: currentOrder.isConversion,
                    couponDiscount: currentOrder.couponDiscount,
                    promotionApplied: currentOrder.promotion != null,
                    creditApplied: currentOrder.creditApplied,
                    isPartialPayment: currentOrder.isPartiallyPaid,
                    onCopy: () => _copyAmount(paymentAmount),
                  ),
                  ...
                  const SizedBox(height: 20),
                  _statusRow(streamAsync, currentOrder),
                  const SizedBox(height: 20),
                  FilledButton.icon(
                    onPressed: () => _showClaimDialog(currentOrder.id),
                    icon: const Icon(Icons.check_circle),
                    label: Text(
                      currentOrder.isPartiallyPaid
                          ? appLocalizations.vPayContinuePaymentWithHash
                          : appLocalizations.vPayIHavePaidWithHash,
                    ),
                  ),
                  const SizedBox(height: 8),
                  OutlinedButton.icon(
                    onPressed: () async {
                      final uri = Uri.parse('https://t.me/verstro_chat');
                      if (await canLaunchUrl(uri)) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              appLocalizations.vPayTelegramNotInstalled,
                            ),
                          ),
                        );
                      }
                  ...
                  const SizedBox(height: 8),
                  Text(
                    appLocalizations.vSupportPublicGroupPrivacy,
                  ...
                  TextButton.icon(
                    onPressed: () async {
                      final uri = Uri.parse('mailto:feedback@verstro.com');
                      if (await canLaunchUrl(uri)) {
                        await launchUrl(
                          uri,
                          mode: LaunchMode.externalApplication,
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(appLocalizations.vHelpOpenLinkFailed),
                          ),
                        );
                      }
                    },
                    icon: const Icon(Icons.email_outlined),
                    label: const Text('feedback@verstro.com'),
                  ),
                  Text(
                    appLocalizations.vSupportFeedbackPrivacy,
                    style: Theme.of(context).textTheme.bodySmall?.copyWith(
                      color: Theme.of(context).colorScheme.onSurfaceVariant,
                    ),
                  ),
  ...
  Widget _statusRow(AsyncValue<OrderDto> streamAsync, OrderDto currentOrder) {
    ...
    if (streamAsync.isLoading) {
      ...
      icon = Icons.wifi_off;
      color = scheme.error;
    } else if (currentOrder.isPartiallyPaid) {
      label = appLocalizations.vClaimPartiallyPaid(
        '\$${centsToUsd(currentOrder.receivedCents)}',
        currentOrder.paymentCount,
        '\$${centsToUsd(currentOrder.remainingCents)}',
      );
  ...
  String _planLabel(String planId) => localizedPlanName(planId, planId);
}
...
class _AmountCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool hasDiscount =
        !isPartialPayment && (couponDiscount != null || creditApplied != null);
      ...
      child: Column(
        ...
        children: [
          if (hasDiscount) ...[
            _AmountBreakdownRow(
              label: isConversion
                  ? appLocalizations.vPayConversionBasePriceLabel
                  : appLocalizations.vPayBasePriceLabel,
              value: '\$$basePrice',
            ),
            if (couponDiscount != null)
              _AmountBreakdownRow(
                label: promotionApplied
                    ? appLocalizations.vPayPromotionDiscountLabel
                    : appLocalizations.vPayCouponDiscountLabel,
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Text(
                appLocalizations.vPayTronAddressTitle,
...
class _SuccessScreen extends ConsumerWidget {
  final OrderDto order;
  final String? claimNotice;

  const _SuccessScreen({required this.order, this.claimNotice});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final paymentState =
        entitlements.value?.paymentState(order.id) ?? 'sync_pending';
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                entitlements.hasError
                    ? appLocalizations.vEntError
                    : entitlementStatusLabel(paymentState),
              ),
              TextButton(
                onPressed: () => invalidateAccount(ref.invalidate),
                child: Text(appLocalizations.vAcctRefresh),
              ),
              if (claimNotice != null) ...[
                const SizedBox(height: 8),
                Text(
      ...
      appBar: AppBar(
        title: Text(
          (serverConfirmed
              ...
              : appLocalizations.vPayLocalExpiryTitle),
        ),
      ),
      body: Center(
        child: Padding(
            ...
            children: [
              ...
              Text(
                (serverConfirmed
                    ...
                    : appLocalizations.vPayLocalExpiryDesc),
                style: Theme.of(context).textTheme.bodyMedium,
                textAlign: TextAlign.center,
              ),
              const SizedBox(height: 16),
              ...
              FilledButton(
                ...
                child: Text(
                  (serverConfirmed
                      ? appLocalizations.vPayBackToReorder
                      : appLocalizations.vPayBackToHome),
                ),
              ),
            ],
          ),
