...
class _VerstroPlanPickerPageState extends ConsumerState<VerstroPlanPickerPage> {
  ...
  Future<void> _pickPlan(PlanDto plan) async {
    ...
    try {
      ...
      final order = await createOrder(ref, plan.id)
    ...
    return Scrollbar(
      ...
      child: Column(
        ...
        children: [
          ...
          // 自适应布局: 宽屏 (>700) 水平 3 列; 窄屏 (mobile) 垂直叠
          LayoutBuilder(
            builder: (context, constraints) {
              final wide = constraints.maxWidth > 700;
              if (wide) {
                // IntrinsicHeight 必须包 Row(crossAxisAlignment: stretch),
                // 否则 Row 拿到 unbounded vertical, 报 "BoxConstraints forces infinite height".
      ...
      child: InkWell(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 6),
            const _Feature(icon: Icons.devices, text: '多设备同时使用'),
            const SizedBox(height: 6),
            const _Feature(icon: Icons.support_agent, text: 'Telegram 客服'),
