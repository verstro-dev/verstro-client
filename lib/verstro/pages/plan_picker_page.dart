...
bool purchaseNeedsUpgrade(PlanDto plan, AccountEntitlements snapshot) {
  const ranks = {'standard': 1, 'premium': 2};
  final target = ranks[plan.squadTier];
  if (target == null ||
      snapshot.revision.trim().isEmpty ||
      !const {
        'none',
        'active',
        'pending_sync',
  ...
  for (final item in [...snapshot.current, ...snapshot.pending]) {
    ...
    if (item.remainingBytes == 0 || item.remainingServiceSeconds == 0) continue;
    if (!const {
      'order',
      'card',
      'conversion',
...
class _VerstroPlanPickerPageState extends ConsumerState<VerstroPlanPickerPage> {
  ...
  Future<void> _pickPlan(PlanDto plan) async {
    ...
    try {
      ...
      if (!current()) return;
      if (e.code == 'entitlement_upgrade_required') {
        setState(() => _error = appLocalizations.vPurchaseUpgradeRequired);
      } else if (e.code == 'purchase_entitlements_unverified') {
        setState(
  ...
  Widget _buildPlans(BuildContext context, List<PlanDto> plans, String email) {
    ...
    final unknown = plans
        ...
        .toList();
    final currentService = ref
        .watch(accountEntitlementsProvider)
        .value
        ?.currentService;
