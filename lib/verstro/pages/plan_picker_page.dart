...
import '../util/purchase_route.dart';
...
import 'entitlement_conversion_page.dart';
...
({String? couponError, String? pageError}) classifyPromotionQuoteError(
  BackendException error, {
  required String couponCode,
}) {
  if (couponCode.isNotEmpty && error.code == 'invalid_coupon') {
    return (couponError: error.message, pageError: null);
  }
  return (couponError: null, pageError: error.message);
}
...
class _VerstroPlanPickerPageState extends ConsumerState<VerstroPlanPickerPage> {
  late bool _independent = widget.independent;
  ...
  GlobalKey _couponFieldKey = GlobalKey(); // 出错时把券输入框滚到可见
  final Map<int, GlobalKey> _promotionExposureKeys = {};
  final Set<String> _promotionImpressions = {};
  final Set<String> _promotionViews = {};
  bool _promotionVisibilityCheckScheduled = false;
  ...
  Future<bool> _confirmIndependentRepurchase() async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(appLocalizations.vPurchaseHistoryReviewTitle),
        content: Text(appLocalizations.vPurchaseHistoryReviewBody),
        actions: [
          TextButton(
          ...
          FilledButton(
            ...
            child: Text(appLocalizations.vPurchaseHistoryReviewOk),
          ),
        ],
      ),
    );
  ...
  Future<PromotionQuoteDto?> _loadQuote(PlanDto plan) async {
    ...
    try {
      ...
      setState(() {
        _couponError = presentation.couponError;
        _error = presentation.pageError;
      });
      rethrow;
    } finally {
  ...
  Future<OrderDto> _createOrderWithPromotion(
    ...
    try {
      ...
      if (!current()) throw StateError('Purchase identity changed');
      if (purchaseMode != 'independent') {
        // 营销等待/重试期间权益可能已变化，普通 POST 前再次核实，变化必须重新选择。
        final latest = await api.getAccountEntitlements();
        if (!current()) throw StateError('Purchase identity changed');
        if (purchaseNeedsUpgrade(plan, latest)) {
          throw const FormatException(
            'Purchase route changed; select the plan again',
          );
        }
      }
      if (!current()) throw StateError('Purchase identity changed');
      ...
      if (current()) {
        invalidateAccount(ref.invalidate);
        ref.invalidate(orderDetailStreamProvider);
      }
      return order;
    } on BackendException catch (e) {
      if (current() &&
        ...
        if (!current()) rethrow;
        return _createOrderWithPromotion(
          plan,
          fresh,
          current: current,
          purchaseMode: purchaseMode,
  ...
  Future<void> _pickPlan(
    PlanDto plan, {
    bool allowIndependentConfirm = true,
  }) async {
    ...
    final uid = auth.value?.user?.id;
    if (auth.isLoading || auth.hasError || uid == null) return;
    if (!widget.independent &&
        !independentPurchaseVisible(
          ref.read(accountEntitlementsProvider).value,
        )) {
      _independent = false;
    }
    final purchaseMode = _independent ? 'independent' : 'automatic';
    final generation = ++_purchaseGeneration;
    bool current() {
      if (!mounted || generation != _purchaseGeneration) return false;
      final now = ref.read(authNotifierProvider);
      return !now.isLoading && !now.hasError && now.value?.user?.id == uid;
    }
    ...
    try {
      // 每次点击都重新读取，不能依赖账户页缓存，也不以订阅摘要代替逐笔权益。
      if (!_independent) {
        final api = await ref.read(backendApiProvider.future);
        if (!current()) return;
        final snapshot = await api.getAccountEntitlements();
        if (!current()) return;
        if (purchaseNeedsUpgrade(plan, snapshot)) {
          if (!mounted) return;
          await Navigator.of(context).push(
            MaterialPageRoute(
              builder: (_) => EntitlementConversionPage(
                initialTargetTier: plan.squadTier,
                initialCouponCode: _couponCode,
                initialUseCredit: true,
                intentOwnerId: uid,
              ),
            ),
          );
      ...
      if (!current()) return;
      final order = await _createOrderWithPromotion(
        plan,
        quote,
        current: current,
        purchaseMode: purchaseMode,
      ...
      if (!current()) return;
      final kind = classifyPurchaseOrderError(e.code);
      if (kind == PurchaseOrderErrorKind.historyReviewRequired &&
          allowIndependentConfirm &&
          !_independent) {
        setState(() => _busyPlanId = null);
        final confirmed = await _confirmIndependentRepurchase();
        if (confirmed && current()) {
          setState(() => _independent = true);
          await _pickPlan(plan, allowIndependentConfirm: false);
          return;
        }
        if (!current()) return;
        setState(() => _error = appLocalizations.vPurchaseHistoryReview);
      } else if (kind == PurchaseOrderErrorKind.historyReviewRequired) {
        setState(() => _error = appLocalizations.vPurchaseHistoryReview);
      } else if (kind == PurchaseOrderErrorKind.upgradeRequired) {
        setState(() => _error = appLocalizations.vPurchaseUpgradeRequired);
      } else if (kind == PurchaseOrderErrorKind.entitlementsUnverified) {
        setState(
          () => _error = appLocalizations.vPurchaseEntitlementsUnverified,
        );
      } else if (e.code == 'invalid_coupon') {
      ...
      if (!current()) return;
      setState(() => _error = appLocalizations.vPurchaseEntitlementsUnverified);
    } catch (e) {
      if (!current()) return;
      setState(() => _error = appLocalizations.vPlanCreateOrderFailed('$e'));
    } finally {
      if (current()) setState(() => _busyPlanId = null);
    }
  }
  ...
  void _recordPromotionEvent(PromotionSummaryDto promotion, String eventType) {
    final token = promotion.exposureToken;
    final dedupeKey = '$eventType:${promotion.campaignId}:$token';
    final recorded = eventType == 'impression'
        ? _promotionImpressions
        : _promotionViews;
    if (token.isEmpty || !recorded.add(dedupeKey)) return;
    ref
        .read(backendApiProvider.future)
        .then(
          (api) => api.recordPromotionEvent(
            eventType: eventType,
            exposureToken: token,
            channel: 'app',
          ),
        ...
        .catchError((_) {
          // 曝光分析永远不能阻断套餐选择；失败允许本次页面生命周期内重试。
          recorded.remove(dedupeKey);
        });
  }

  void _schedulePromotionVisibilityCheck(List<PromotionSummaryDto> promotions) {
    if (_promotionVisibilityCheckScheduled || promotions.isEmpty) return;
    _promotionVisibilityCheckScheduled = true;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _promotionVisibilityCheckScheduled = false;
      if (!mounted) return;
      final viewportHeight = MediaQuery.sizeOf(context).height;
      for (final promotion in promotions) {
        final itemContext =
            _promotionExposureKeys[promotion.campaignId]?.currentContext;
        final box = itemContext?.findRenderObject();
        if (box is! RenderBox || !box.attached || !box.hasSize) continue;
        final top = box.localToGlobal(Offset.zero).dy;
        final bottom = top + box.size.height;
        if (bottom > 0 && top < viewportHeight) {
          _recordPromotionEvent(promotion, 'impression');
        }
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ref.listen(authNotifierProvider, (previous, next) {
      if (!identical(previous, next)) {
        ...
        _quoteRequest = null;
        _couponCode = '';
        _couponFieldKey = GlobalKey();
        if (previous?.value?.user?.id != null &&
            previous?.value?.user?.id != next.value?.user?.id) {
          _independent = false;
        }
        _error = null;
        _couponError = null;
  ...
  Widget _buildPlans(BuildContext context, List<PlanDto> plans, String email) {
    ...
    _schedulePromotionVisibilityCheck(activePromotions);
    final promotionPlanIds = activePromotions
        .expand((promotion) => promotion.planIds)
        .toSet();
    // 标准/专业分组展示 (plan 1-2): 旧后端无专业档则不分组, 直接铺.
    final standard = plans.where((p) => p.squadTier == 'standard').toList();
    final premium = plans.where((p) => p.squadTier == 'premium').toList();
    final unknown = plans
        .where((p) => p.squadTier != 'standard' && p.squadTier != 'premium')
        .toList();
    final entitlements = ref.watch(accountEntitlementsProvider).value;
    final currentService = entitlements?.currentService;
    final showIndependent = widget.independent ||
        independentPurchaseVisible(entitlements);
    final renewalStatus = renewalServiceStatus(currentService?.status ?? '');
    final credit = ref.watch(creditProvider).asData?.value;
    final honesty = _quote == null
        ? null
        : creditHonesty(
            purchaseBalanceCents: credit?.purchaseBalanceCents ?? 0,
            creditAllowed: _quote!.creditAllowed,
            priceAfterDiscountCents: _quote!.priceAfterDiscountCents,
          );
    final renewingPremium =
        currentService?.status == 'active' &&
        currentService?.squadTier == 'premium';
    final List<Widget> planSections = premium.isEmpty && unknown.isEmpty
        ...
        ? [
            ...
            if (unknown.isNotEmpty) ...[
              if (standard.isNotEmpty) const SizedBox(height: 20),
              _tierHeader(
                context,
                appLocalizations.vPlanTierUnknown,
                appLocalizations.vPlanTierUnknownDesc,
              ),
              _planGrid(unknown, promotionPlanIds),
            ],
          ]
        : renewingPremium
        ? [
            ...
            _planGrid(premium, promotionPlanIds, renewingPremium: true),
            if (standard.isNotEmpty) ...[
              const SizedBox(height: 16),
              ExpansionTile(
                key: const ValueKey('other-plans'),
                tilePadding: EdgeInsets.zero,
                title: Text(appLocalizations.vPlanOtherPlans),
                subtitle: Text(appLocalizations.vPlanStandardAfterPremium),
                children: [
                  _tierHeader(
                    context,
                    appLocalizations.vPlanTierStandard,
          ...
          child: Column(
            ...
            children: [
              ...
              if (renewalStatus != null) ...[
                Card(
                  key: const ValueKey('renewal-status'),
                  child: ListTile(
                    leading: Icon(
                      Icons.info_outline,
                      color: Theme.of(context).colorScheme.primary,
                    ),
                    title: Text(switch (renewalStatus) {
                      'expired' => appLocalizations.vPlanRenewalExpired,
                      'exhausted' => appLocalizations.vPlanRenewalExhausted,
                      _ => appLocalizations.vPlanRenewalNone,
                    }),
                  ),
                ),
              ...
              if (showIndependent)
                SwitchListTile(
                  key: const ValueKey('purchase-independent'),
                  title: Text(appLocalizations.vEntIndependent),
                  subtitle: Text(
                    _independent
                        ? appLocalizations.vPurchaseIndependentHint
                        : appLocalizations.vPurchaseAutomaticHint,
                  ),
                  value: _independent,
                      ...
                      children: [
                        ...
                        ...activePromotions.map(
                          (promotion) => Semantics(
                            button: true,
                            child: InkWell(
                              key: _promotionExposureKeys.putIfAbsent(
                                promotion.campaignId,
                                GlobalKey.new,
                              ),
                              onTap: () =>
                                  _recordPromotionEvent(promotion, 'view'),
                              child: Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    FilledButton.tonal(
                      ...
                      onPressed: _quoting || _busyPlanId != null
                          ? null
                          : () => _applyCoupon(plans),
                      child: _quoting
  ...
  Widget _planGrid(
    List<PlanDto> plans,
    Set<String> promotionPlanIds, {
    bool renewingPremium = false,
  }) {
                    ...
                    (e) => [
                      Expanded(
                        child: _PlanCard(
                          ...
                          renewingPremium: renewingPremium,
                        ),
                      ),
...
class PromotionQuoteBreakdownCard extends StatelessWidget {
  ...
  final CreditHonesty? honesty;

  const PromotionQuoteBreakdownCard({
    super.key,
    required this.quote,
    this.honesty,
  });
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              if (honesty?.showBlockedNote == true)
                Padding(
                  padding: const EdgeInsets.only(top: 8),
                  child: Text(
                    appLocalizations.vPromotionCreditBlocked,
                    textAlign: TextAlign.center,
                  ),
                ),
              if (honesty?.showPreviewDeduction == true) ...[
                _PromotionAmountRow(
                  label: appLocalizations.vPayCreditAppliedLabel,
                  value: '−${_usd(honesty!.previewCreditCents)}',
                ),
                _PromotionAmountRow(
                  label: appLocalizations.vPromotionCreditPreview,
                  value: _usd(honesty!.previewPayableCents),
                  emphasized: true,
                ),
              ],
            ],
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 6),
              if (const {'standard', 'premium'}.contains(plan.squadTier)) ...[
                _Feature(
                  icon: plan.squadTier == 'premium' ? Icons.tune : Icons.bolt,
                  text: plan.squadTier == 'premium'
