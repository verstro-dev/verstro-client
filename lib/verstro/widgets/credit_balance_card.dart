  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return credit.when(
      ...
      error: (_, _) => alwaysShow
          ...
          : const SizedBox.shrink(),
      data: (c) {
        if (!alwaysShow && c.purchaseBalanceCents <= 0) {
          return const SizedBox.shrink();
        }
        final amount = '\$${centsToUsd(c.purchaseBalanceCents)}';
        return Column(
            ...
            Padding(
              ...
              child: Card(
                child: ListTile(
                  ...
                  subtitle: Text(
                    c.canReusePendingFunds
                        ? appLocalizations.vAcctPurchaseBalanceSubtitle
                        : appLocalizations.vAcctBalanceSubtitle,
                  ),
                ),
              ),
            ),
            if (alwaysShow && c.conversionHeldCents > 0)
              Card(
                child: ListTile(
                  title: Text(appLocalizations.vConvHeld),
                  subtitle: Text('${centsToUsd(c.conversionHeldCents)} USDT'),
                ),
              ),
            if (alwaysShow &&
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        c.canReusePendingFunds
                            ? appLocalizations.vAcctReusablePendingSubtitle
                            : appLocalizations.vAcctPendingSubtitle,
                      ),
                      for (final order in c.pendingOrders)
                        _PendingOrderRow(
                          key: ValueKey(order.orderId),
                          order: order,
                          reusable: c.canReusePendingFunds,
                        ),
                    ],
                  ),
                ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (_expired)
            Text(
              widget.reusable
                  ? appLocalizations.vAcctReusablePendingExpired
                  : appLocalizations.vAcctPendingTransfer,
            )
          else
            TextButton(
