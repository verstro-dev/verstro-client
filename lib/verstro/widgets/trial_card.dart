...
class _TrialCardState extends ConsumerState<TrialCard> {
  ...
  Future<void> _claim() async {
    ...
    try {
      ...
      _toast(appLocalizations.vTrialActivated);
    } on BackendException catch (e) {
      ref.invalidate(trialStatusProvider); // 纠正状态(已领/未开放等)
      _toast(e.message);
    } catch (e) {
      _toast(appLocalizations.vTrialClaimFailed);
    } finally {
    ...
    return tsAsync.maybeWhen(
      data: (ts) {
        ...
        final title = Row(children: [
          ...
          Text(appLocalizations.vTrialTitle, style: Theme.of(context).textTheme.titleMedium),
        ]);
        return Padding(
              ...
              child: Column(
                ...
                children: emailVerified
                    ? [
                        ...
                        Text(appLocalizations.vTrialSpec(ts.days, ts.trafficGb),
                        ...
                        SizedBox(
                          ...
                          child: FilledButton(
                            ...
                            child: _busy
                                ...
                                : Text(appLocalizations.vTrialClaimNow),
                          ),
                        ),
                      ]
                    : [
                        title,
                        const SizedBox(height: 4),
                        Text(appLocalizations.vTrialVerifyEmailHint(ts.days),
                            style: Theme.of(context).textTheme.bodyMedium),
                        const SizedBox(height: 12),
                        // 内联验证码表单 (plan 1-2): 无订阅用户只看到本卡, 必须能在此输码验证
