...
class _VerstroEmailVerifyFormState extends ConsumerState<VerstroEmailVerifyForm> {
  ...
  Future<void> _verify() async {
    ...
    if (code.isEmpty) {
      setState(() => _error = appLocalizations.vAuthCodeRequired);
      return;
    }
    setState(() {
      _busy = true;
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(appLocalizations.vAuthEmailVerified)),
      );
      widget.onVerified?.call();
    } on BackendException catch (e) {
      if (mounted) setState(() => _error = e.message);
    } catch (_) {
      if (mounted) setState(() => _error = appLocalizations.vAuthVerifyFailedNetwork);
    } finally {
  ...
  Future<void> _resend() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(appLocalizations.vAuthCodeSent)),
      );
      _startCooldown(60);
    } on BackendException catch (e) {
      if (mounted) setState(() => _error = e.message);
    } catch (_) {
      if (mounted) setState(() => _error = appLocalizations.vAuthSendFailedRetry);
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  }
          ...
          children: [
            ...
            FilledButton(
              ...
              child: _busy
                  ...
                  : Text(appLocalizations.vAuthVerifyButton),
            ),
          ],
        ),
        if (_error != null) ...[
        ...
        Align(
          ...
          child: TextButton(
            ...
            child: Text(_cooldown > 0
                ...
                : appLocalizations.vAuthResendCodeButton),
          ),
        ),
      ],
    );
