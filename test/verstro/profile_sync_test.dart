...
void main() {
  ...
  group('planVerstroProfile', () {
    ...
    test('forceRefresh 且 url 相同 → refresh 保留并清多余', () {
      final r = planVerstroProfile(
        2,
        [p(1, 'A'), p(2, 'U')],
        'U',
        forceRefresh: true,
      );
      expect(r.action, VerstroProfileAction.refresh);
      expect(r.keepId, 2);
      expect(r.deleteIds, [1]);
    });

    test('forceRefresh 但 url 变了仍是 replace', () {
      final r = planVerstroProfile(1, [p(1, 'OLD')], 'NEW', forceRefresh: true);
      expect(r.action, VerstroProfileAction.replace);
      expect(r.keepId, 1);
    });
  });
