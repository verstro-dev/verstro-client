...
class _AccountSnapshot extends AccountEntitlementsNotifier {
  _AccountSnapshot(this.subscription);
  final SubscriptionDto subscription;
  @override
  Future<AccountEntitlements?> build() async => AccountEntitlements.fromJson({
    'as_of': '2026-09-09T00:00:00Z',
    'revision': 'fixture',
    'history_revision': 'history-fixture',
    'current_service': {'status': 'none'},
    'current': [
      for (var i = 0; i < subscription.grants.length; i++)
        {
          'grant_id': i + 1,
          'source_kind': 'order',
          'source_order_id': i + 101,
          'plan_id': subscription.grants[i].planId,
          'plan_name': subscription.grants[i].planName,
          'squad_tier': 'standard',
          'status': 'active',
          'starts_at': '2026-09-09T00:00:00Z',
          'active_until': '2026-10-09T00:00:00Z',
          'remaining_service_seconds': 100,
          'quota_bytes': 1024,
          'consumed_bytes': 0,
...
class _RouteSnapshot extends AccountEntitlementsNotifier {
  _RouteSnapshot(this.onRead);
...
Future<void> pumpAccountPage(
  WidgetTester tester,
  SubscriptionDto sub, {
  Future<CreditDto> Function()? fetchCredit,
}) async {
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        ...
        accountEntitlementsProvider.overrideWith(() => _AccountSnapshot(sub)),
        ...
        creditProvider.overrideWith(
          (ref) async => fetchCredit != null
              ? await fetchCredit()
              : const CreditDto(balanceCents: 0, credits: []),
        ),
...
void main() {
  ...
  for (final route in ['Orders and entitlements', 'My devices']) {
    for (final action in ['return', 'pull']) {
      testWidgets('$route $action 联动重读账户权益订阅订单资金设备', (tester) async {
        await tester.binding.setSurfaceSize(const Size(800, 1800));
        addTearDown(() => tester.binding.setSurfaceSize(null));
        var generation = 0;
        final reads = <String, List<int>>{};
        void record(String key) =>
            reads.putIfAbsent(key, () => []).add(generation);
        final container = ProviderContainer(
          overrides: [
            authNotifierProvider.overrideWith(_FakeAuthNotifier.new),
            accountEntitlementsProvider.overrideWith(
              () => _RouteSnapshot(() => record('entitlements')),
            ),
            ...
            ordersListProvider.overrideWith((ref) async {
              ...
              return [
                historyOrder(
                  id: 601,
                  status: generation == 0 ? 'waiting' : 'finished',
            ...
            creditProvider.overrideWith((ref) async {
              record('credit');
              return CreditDto(balanceCents: generation, credits: const []);
            }),
            devicesListProvider.overrideWith((ref) async {
              record('devices');
              return DevicesInfo(devices: const [], maxDevices: generation + 5);
            }),
            currentDeviceIdProvider.overrideWith((ref) async => 'synthetic'),
            agentProvider.overrideWith((ref) async => _fakeAgent),
            myPromotionsProvider.overrideWith(
              (ref) async => const MyPromotionsState(supported: true),
            ),
        ...
        Future<void> readAll() async {
          await Future.wait<dynamic>([
            container.read(accountEntitlementsProvider.future),
            container.read(subscriptionProvider.future),
            container.read(ordersListProvider.future),
            container.read(creditProvider.future),
            container.read(devicesListProvider.future),
          ]);
        }
        ...
        await tester.pumpAndSettle();
        generation = 1; // 模拟其他端支付完成或后台权益/权限变化。
        if (action == 'pull') {
          ...
          for (final key in [
            ...
            expect(reads[key]!.last, 1, reason: '二级页下拉未刷新 $key');
          }
          generation = 2;
        }
        Navigator.of(
          tester.element(
            find.byType(
              route == 'Orders and entitlements'
                  ? VerstroOrderHistoryPage
                  : VerstroDevicesPage,
            ),
          ),
        ).pop();
        ...
        for (final key in [
          ...
          expect(reads[key]!.last, generation, reason: '返回账户未刷新 $key');
        }
        expect(
          (await container.read(ordersListProvider.future)).single.status,
          'finished',
        );
        await tester.pumpWidget(const SizedBox.shrink());
      });
  ...
  testWidgets('订阅卡: grant.planName 走 localizedPlanName, 不泄漏后端中文名', (
    ...
    expect(find.textContaining('Standard · Monthly · #1'), findsOneWidget);
    expect(find.textContaining('Pro · Yearly · #2'), findsOneWidget);
  });
  ...
  testWidgets('finished 和 failed 终态不可恢复发票页', (tester) async {
    for (final status in ['finished', 'failed']) {
      ...
      final tileFinder = find.ancestor(
        of: find.text('#${terminal.id}'),
        matching: find.byType(InkWell),
      );
      expect(tileFinder, findsOneWidget);
      final tile = tester.widget<InkWell>(tileFinder);
  ...
  testWidgets('下拉刷新重新获取credit并等待完成', (tester) async {
    ...
    await pumpAccountPage(
      tester,
      SubscriptionDto.fromJson({'has_subscription': false}),
      fetchCredit: () async {
        reads++;
  ...
  testWidgets('下拉刷新请求失败保留provider错误，不抛出未处理异常', (tester) async {
    ...
    final container = ProviderScope.containerOf(
      tester.element(find.byType(VerstroAccountPage)),
    );
