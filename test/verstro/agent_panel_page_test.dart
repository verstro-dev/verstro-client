...
void main() {
  ...
  group('钱包卡四态', () {
    ...
    testWidgets('processing=0: 处理中行不渲染, 提现按钮正常', (tester) async {
      await pumpPanel(
        tester,
        agent(availableCents: 2000, minPayoutCents: 1000),
      );
  ...
  group('正式合作伙伴授权', () {
    testWidgets('战略分销伙伴显示认证名称、授权编号和非独家边界', (tester) async {
      await pumpPanel(
        tester,
        agent(
          tier: 'master',
      ...
      expect(find.text('Verstro 平台认证'), findsOneWidget);
      expect(find.text('授权编号 VPA-23456789'), findsOneWidget);
      expect(find.text('标准合作默认为非独家'), findsOneWidget);
    });
  });
  ...
  group('提现门槛动态化', () {
    testWidgets('min_payout_cents=2000 → 未满文案显示 \$20.00', (tester) async {
      await pumpPanel(
        tester,
        agent(availableCents: 1500, minPayoutCents: 2000),
      );
      expect(find.text('提现到 TRC20'), findsNothing);
    ...
    testWidgets('可提现恰好达 min → 显示提现按钮', (tester) async {
      await pumpPanel(
        tester,
        agent(availableCents: 2000, minPayoutCents: 2000),
      );
      expect(find.text('提现到 TRC20'), findsOneWidget);
    });
  });
