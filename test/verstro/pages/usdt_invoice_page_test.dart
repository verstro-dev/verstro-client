...
Future<_FakeBackendApi> _pumpInvoice(
  ...
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        if (conversionPayment) authNotifierProvider.overrideWith(TestAuth.new),
...
void main() {
  for (final english in [false, true]) {
    for (final conversion in [true, false]) {
      testWidgets('付款金额标签与套餐名 en=$english conversion=$conversion', (
        tester,
      ) async {
        final now = DateTime.now().toUtc();
        final invoice = OrderDto.fromJson({
          'id': 96,
          'purpose': conversion ? 'conversion_funding' : 'ordinary',
          if (conversion)
            'conversion_id': '11111111-1111-4111-8111-111111111111',
          'plan_id': 'premium-quarterly',
          'base_price': '15.39',
          'coupon_discount': '5.75',
          'final_amount': '10.36',
          'remaining_cents': 1036,
          'status': 'waiting',
          'deposit_address': 'TSyntheticPaymentAddress',
          'created_at': now.toIso8601String(),
          'expires_at': now.add(const Duration(hours: 1)).toIso8601String(),
        });
        await _pumpInvoice(
          tester,
          initialOrder: invoice,
          claimSteps: [],
          conversionPayment: conversion,
          locale: english ? const Locale('en') : const Locale('zh', 'CN'),
        );
        final plan = english ? 'Pro · Quarterly' : '专业·季付';
        expect(find.textContaining(plan), findsOneWidget);
        expect(find.textContaining('premium-quarterly'), findsNothing);
        final ordinaryLabel = english ? 'Original price' : '原价';
        final conversionLabel = english
            ? 'After trade-in (before discounts)'
            : '折价抵扣后金额（优惠前）';
        expect(
          find.text(conversion ? conversionLabel : ordinaryLabel),
          findsOneWidget,
        );
        expect(
          find.text(conversion ? ordinaryLabel : conversionLabel),
          findsNothing,
        );
        expect(find.text('\$15.39'), findsOneWidget);
  ...
  test('换购基价标签英文生成文案', () async {
    final l = await AppLocalizations.load(const Locale('en'));
    expect(l.vPayConversionBasePriceLabel, 'After trade-in (before discounts)');
    expect(l.vPayBasePriceLabel, 'Original price');
    await AppLocalizations.load(const Locale('zh', 'CN'));
  });

  testWidgets('历史订单首次核验未完成或失败时不展示付款二维码', (tester) async {
    await AppLocalizations.load(const Locale('zh', 'CN'));
    final stream = StreamController<OrderDto>();
    ...
    await tester.pumpWidget(
      ProviderScope(
        ...
        child: MaterialApp(home: VerstroUsdtInvoicePage(order: order())),
      ),
    );
    await tester.pump();
    ...
    await tester.pump();
    expect(find.byType(QrImageView), findsNothing);
    stream.add(order(status: 'expired'));
    await tester.pump();
    expect(find.byType(QrImageView), findsNothing);
    await tester.pumpWidget(const SizedBox.shrink());
  });
