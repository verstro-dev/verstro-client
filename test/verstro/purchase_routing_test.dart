...
class _PickerEntitlements extends AccountEntitlementsNotifier {
  _PickerEntitlements(this.initial);
  final AccountEntitlements? initial;
  @override
  Future<AccountEntitlements?> build() async => initial;
  void replace(AccountEntitlements? value) => state = AsyncData(value);
}

class RoutingAPI extends fixtures.EntitlementsAPI {
  RoutingAPI(super.token);
  int orders = 0, resources = 0;
  bool orderSucceeds = false, expireFirst = false, failOnlyAutomatic = false;
  ...
  Future<OrderDto> createOrder(
    ...
    if (orderFailure != null &&
        (!failOnlyAutomatic || purchaseMode != 'independent')) {
      throw orderFailure!;
    }
    if (orderSucceeds) {
      return OrderDto(
...
Future<ProviderContainer> mount(
  ...
  final container = ProviderContainer(
    overrides: [
      ...
      plansProvider.overrideWith((ref) async => targets ?? [target ?? plan()]),
      accountEntitlementsProvider.overrideWith(
        () => _PickerEntitlements(entitlements),
      ),
...
void main() {
  for (final withStandard in [false, true]) {
    testWidgets(
      '${withStandard ? 'standard+unknown' : 'unknown-only'} 目录保留未知档位中性组',
      (tester) async {
        final api = RoutingAPI(
          TokenStorage(await SharedPreferences.getInstance()),
        );
        final unknown = PlanDto(
          squadTier: 'future-tier',
          id: 'future-only',
          name: 'Future capability plan',
          durationDays: 30,
          priceUsd: '7.00',
        ...
        await mount(
          ...
          api,
          targets: [
            if (withStandard) plan(tier: 'standard'),
            unknown,
          ],
        );
        expect(find.text('Future capability plan'), findsOneWidget);
        expect(
          find.text('Plan capabilities pending confirmation'),
          findsOneWidget,
        );
        final card = find.ancestor(
          of: find.text('Future capability plan'),
          matching: find.byType(Card),
        );
        expect(
          find.descendant(
            of: card,
            matching: find.text('Auto-selects the fastest node'),
          ),
          findsNothing,
        );
      },
  ...
  testWidgets('当前 active premium 按权威档位专业优先且标准仍可展开', (tester) async {
    final api = RoutingAPI(TokenStorage(await SharedPreferences.getInstance()));
    final premium = PlanDto(
      squadTier: 'premium',
      id: 'pro-without-prefix',
      name: 'Authoritative Pro',
      durationDays: 30,
      priceUsd: '6.00',
      trafficLimitBytes: 1024,
    );
    final standard = PlanDto(
      squadTier: 'standard',
      id: 'premium-misleading-id',
      name: 'Authoritative Standard',
      durationDays: 30,
      priceUsd: '6.00',
      trafficLimitBytes: 1024,
    );
    ...
    final container = await mount(
      tester,
      api,
      targets: [standard, premium, unknown],
      entitlements: AccountEntitlements.fromJson(fixtures.snapshot()),
      home: const VerstroPlanPickerPage(),
    );
    expect(find.text('Authoritative Pro'), findsOneWidget);
    expect(find.text('Unknown capabilities'), findsOneWidget);
    expect(find.text('Plan capabilities pending confirmation'), findsOneWidget);
    expect(find.text('Authoritative Standard'), findsNothing);
    expect(find.byKey(const ValueKey('other-plans')), findsOneWidget);
    await tester.tap(find.byKey(const ValueKey('other-plans')));
    await tester.pumpAndSettle();
    expect(find.text('Authoritative Standard'), findsOneWidget);

    final standardSnapshot = AccountEntitlements.fromJson({
      ...fixtures.snapshot(),
      'current_service': {
        ...fixtures.snapshot()['current_service'] as Map<String, dynamic>,
        'squad_tier': 'standard',
      },
    });
    (container.read(accountEntitlementsProvider.notifier)
            as _PickerEntitlements)
        .replace(standardSnapshot);
    await tester.pumpAndSettle();
    expect(find.byKey(const ValueKey('other-plans')), findsNothing);
    expect(find.text('Unknown capabilities'), findsOneWidget);
  });

  testWidgets('320 宽 1.5 字体可滚动展开其他套餐并点击标准卡', (tester) async {
    final api = RoutingAPI(TokenStorage(await SharedPreferences.getInstance()));
    final standard = PlanDto(
      squadTier: 'standard',
      id: 'standard-narrow',
      name: 'Narrow Standard',
      planVersionId: 9,
      durationDays: 30,
      priceUsd: '6.00',
      trafficLimitBytes: 1024,
    ...
    await mount(
      tester,
      api,
      targets: [standard, premium],
      entitlements: AccountEntitlements.fromJson(fixtures.snapshot()),
      surfaceSize: const Size(320, 900),
      home: const MediaQuery(
        data: MediaQueryData(textScaler: TextScaler.linear(1.5)),
        child: VerstroPlanPickerPage(independent: true),
      ),
    );
    await tester.scrollUntilVisible(
      find.byKey(const ValueKey('other-plans')),
      300,
    ...
    await tester.pumpAndSettle();
    await tester.scrollUntilVisible(
      find.text('Narrow Standard'),
      300,
      scrollable: find.byType(Scrollable).first,
    );
    expect(find.text('Narrow Standard'), findsOneWidget);
    await tester.tap(find.text('Narrow Standard'));
    await tester.pumpAndSettle();
    expect(api.modes, contains('independent'));
  ...
  testWidgets('过期无剩余权益不展示独立购买开关，并显示续费说明', (
    tester,
  ) async {
    final api = RoutingAPI(TokenStorage(await SharedPreferences.getInstance()));
    await mount(
    ...
    expect(find.byKey(const ValueKey('purchase-independent')), findsNothing);
    expect(find.byKey(const ValueKey('renewal-status')), findsOneWidget);
    expect(
      find.text(
  ...
  testWidgets('仍有剩余权益时保留显式独立购买入口', (tester) async {
    final api = RoutingAPI(TokenStorage(await SharedPreferences.getInstance()));
    await mount(
      tester,
      api,
    ...
    expect(find.byKey(const ValueKey('purchase-independent')), findsOneWidget);
  });
  testWidgets('历史待核验需确认后才发独立购买，不静默改模式', (tester) async {
    final api = RoutingAPI(TokenStorage(await SharedPreferences.getInstance()))
      ..orderFailure = const ConflictException(
        'current_service_history_review_required',
        'Opaque Chinese review message',
      )
      ..failOnlyAutomatic = true
      ..orderSucceeds = true;
    OrderDto? received;
    await mount(
      tester,
    ...
    await pick(tester);
    expect(api.modes, ['automatic']);
    expect(find.text('Confirm independent repurchase'), findsOneWidget);
    expect(find.text('Opaque Chinese review message'), findsNothing);
    await tester.tap(find.text('Pay for a new service'));
    await tester.pump();
    await tester.pump();
    expect(api.modes, ['automatic', 'independent']);
    expect(received?.id, 7);
    expect(find.text('Opaque Chinese review message'), findsNothing);
  });
}
