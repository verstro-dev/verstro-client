...
void main() {
  ...
  testWidgets('新后端余额已包含部分到账且支持在套餐页使用', (tester) async {
    final dto = CreditDto.fromJson({
      'balance_cents': 10,
      'credits': null,
      'purchase_balance_cents': 110,
      'reusable_held_cents': 0,
      'pending_order_cents': 100,
      'pending_orders': [
        {
          'order_id': 93,
    ...
    await pump(tester, () async => dto);
    expect(find.text('可用余额  \$1.10'), findsOneWidget);
    expect(find.textContaining('请勿重复相加'), findsOneWidget);
    expect(find.textContaining('暂不可用于其他购买'), findsNothing);
