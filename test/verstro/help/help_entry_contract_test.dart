...
void main() {
  ...
  test('退出登录先拆除代理再清 token，且 dispose 不退出进程', () {
    final auth = File('lib/verstro/providers/auth_provider.dart').readAsStringSync();
    final teardown = auth.indexOf('handleLogoutTeardown');
    final clearToken = auth.indexOf('await api.logout()');
    expect(teardown, greaterThanOrEqualTo(0));
    expect(clearToken, greaterThan(teardown));

    final application = File('lib/application.dart').readAsStringSync();
    expect(application, contains('proxy?.stopProxy()'));
    expect(application, contains('appController.isAttach = false'));
    expect(
      application,
      contains('ApplicationDisposeCleanup.selfCoreAndExit'),
    );
  });

  test('启动在自动连接前应用 autoRun 一次性关闭并清系统代理', () {
    final source = File('lib/state.dart').readAsStringSync();
    final autoRun = source.indexOf('_applyAutoRunOffOnce');
    final heal = source.indexOf('planStartupProxyHeal');
    final stop = source.indexOf('proxy?.stopProxy()');
    expect(autoRun, greaterThanOrEqualTo(0));
    expect(heal, greaterThan(autoRun));
    expect(stop, greaterThan(heal));
  });
