...
void main() {
  ...
  test('桌面系统代理说明包含强制退出后的残留清理', () async {
    final phrases = <Locale, String>{
      const Locale('zh', 'CN'): '再打开一次 Verstro 会清掉残留系统代理',
      const Locale('en'): 'open Verstro once to clear leftover system proxy',
      const Locale('ja'): 'Verstro をもう一度開くと残留したシステムプロキシを解除',
      const Locale('ru'): 'снова откройте Verstro',
    };
    for (final entry in phrases.entries) {
      ...
      final body = content.sections
          .firstWhere((section) => section.id == 'system-proxy')
          .body;
      expect(body, contains(entry.value), reason: '${entry.key}');
    }
  });
