...
void main() {
  ...
  group('buildShareText 通用', () {
    ...
    test('全量: 品牌句 + 试用 + 双边奖励 + 统一域名', () {
      ...
      expect(t, endsWith('下载：https://verstro.com'));
      // 口径: 大众向不出现技术词
  ...
  group('buildShareText 其它风格', () {
    ...
    test('开发者向: TUN/GPLv3 技术卖点只在此风格出现', () {
      ...
      expect(t, contains('brew / docker / git'));
  ...
  group('SharePoster', () {
    ...
    testWidgets('有码: 邀请码/奖励/试用/主标题/域名齐全', (tester) async {
      await tester.pumpWidget(host(const SharePoster(
        code: 'P253MPHE',
        refereeRewardCents: 200,
        referrerRewardCents: 200,
        trialEnabled: true,
      ...
      expect(find.text('一键全局加速'), findsOneWidget);
