...
void main() {
  ...
  test('已知业务码映射到本地化串（非后端透传）', () {
    ...
    expect(localizedBusinessError('email_taken'), '该邮箱已注册'); // 复用 vApiEmailTaken
  ...
  group('businessErrorMessage — _translateBusinessError 消费的组合入口', () {
    ...
    test('404 no_subscription → 本地化 vErrNoSubscription（非后端透传）', () {
      expect(businessErrorMessage('no_subscription', sentinelBackendMessage), '无订阅');
    });

    test('500 provision_failed → 本地化 vErrProvisionFailed（非后端透传）', () {
      expect(businessErrorMessage('provision_failed', sentinelBackendMessage),
          '试用开通失败，请重试');
    });

    test('410 expired → 本地化 vErrSubExpired（此前 410 未 case，落 unexpected status）', () {
      expect(businessErrorMessage('expired', sentinelBackendMessage), '订阅已过期');
    });

    test('429 code_locked → 本地化 vErrCodeLocked（此前 429 未 case，落 unexpected status）', () {
      expect(businessErrorMessage('code_locked', sentinelBackendMessage),
          '尝试次数过多，请重新获取验证码');
    });
  ...
  group('coupon_* reason_code 子码 — Task 7 (invalid_coupon 细分本地化)', () {
    ...
    test('7 个 coupon_* 子码经 localizedBusinessError 命中本地化串', () {
    ...
    test('coupon_* 子码经 businessErrorMessage 覆盖后端 message（非透传，用哨兵串排除巧合）', () {
      expect(businessErrorMessage('coupon_invalid', sentinelBackendMessage), '无效优惠码');
      expect(businessErrorMessage('coupon_disabled', sentinelBackendMessage), '优惠码已停用');
      expect(businessErrorMessage('coupon_inactive', sentinelBackendMessage), '优惠码未开始或已过期');
      expect(businessErrorMessage('coupon_plan_mismatch', sentinelBackendMessage), '优惠码不适用本套餐');
      expect(businessErrorMessage('coupon_new_users_only', sentinelBackendMessage), '仅限新用户');
      expect(businessErrorMessage('coupon_limit_reached', sentinelBackendMessage), '已达每人使用上限');
      expect(businessErrorMessage('coupon_sold_out', sentinelBackendMessage), '优惠码已抢光');
    });

    test('businessErrorMessage(coupon_sold_out, 后端原文) → 本地化覆盖后端 message（非透传）', () {
    ...
    test('向后兼容边界: businessErrorMessage(invalid_coupon, 后端Reason) 顶层 code 未命中 → 透传后端 message', () {
      ...
      expect(businessErrorMessage('invalid_coupon', '该优惠码不适用于当前套餐'),
          '该优惠码不适用于当前套餐');
    });
  });
}
